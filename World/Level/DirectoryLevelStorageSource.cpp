...
#include "../IO/ConsoleSaveFileOriginal.h"
...
bool DirectoryLevelStorageSource::isNewLevelIdAcceptable(const wstring& levelId)
{
	...
	// Create the world folder and required subdirectories
	levelFolder.mkdirs();

	// Ensure region subdirectory exists for chunk storage
	File regionFolder = File(levelFolder, L"region");
	regionFolder.mkdir();
