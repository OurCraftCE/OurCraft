...
enum class ChunkGenStage {
	Empty,           // Allocated but no data
	Terrain,         // Basic terrain shape (blocks placed)
	Caves,           // Caves carved
	Features,        // Trees, ores, structures placed
	Lighting,        // Light calculated
	Complete         // Ready for gameplay
};
...
public:
    ...
    bool loaded;
	ChunkGenStage m_genStage;
	ChunkGenStage getGenStage() const { return m_genStage; }
	void setGenStage(ChunkGenStage stage) { m_genStage = stage; }
	bool isUsableForRendering() const { return m_genStage >= ChunkGenStage::Terrain; }
	bool isFullyGenerated() const { return m_genStage >= ChunkGenStage::Complete; }
