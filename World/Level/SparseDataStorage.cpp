...
void SparseDataStorage::read(DataInputStream *dis)
{
	...
	if (count < 0 || count > 0xffff) return; // guard against integer overflow / malformed data
	unsigned char *dataPointer = (unsigned char *)malloc((size_t)count * 128 + 128);
	byteArray wrapper(dataPointer, (size_t)count * 128 + 128);
