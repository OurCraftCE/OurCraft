...
#include "../Math/SocketAddress.h"
#include "Socket.h"
#include "../Math/ThreadName.h"
#include "../../Client/Network/ServerConnection.h"
#include <algorithm>
...
NetworkAdapter *Socket::s_networkAdapter = NULL;
...
void Socket::SocketInputStreamLocal::close()
{
	...
	(void)s_hostQueue[m_queueIdx].empty();
	LeaveCriticalSection(&s_hostQueueLock[m_queueIdx]);
}
...
void Socket::SocketOutputStreamNetwork::writeWithFlags(byteArray b, unsigned int offset, unsigned int length, int flags)
{
	...
	// Local connections bypass the network and write directly to the other end's queue
	...
	{
		// Send via NetworkAdapter (RakNet) instead of QNet
		if(!Socket::s_networkAdapter) return;
		...
		ConnectionId conn = Socket::s_networkAdapter->getConnectionForPlayer(socketPlayer);
		bool lowPriority = (flags != 0);
		Socket::s_networkAdapter->sendData(conn, &b[offset], length, lowPriority);
	}
}
