...
RandomLevelSource::RandomLevelSource(Level *level, __int64 seed, bool generateStructures) : generateStructures( generateStructures ), m_ownsCS(true)
{
	InitializeCriticalSectionAndSpinCount(&m_csRandom, 4000);
...
RandomLevelSource::~RandomLevelSource()
{
	...
	if (m_ownsCS)
		DeleteCriticalSection(&m_csRandom);
}
...
void RandomLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
    ...
    for (int x = 0; x < 16; x++)
	{
        for (int z = 0; z < 16; z++)
		{
            int worldX = xOffs * 16 + x;
            int worldZ = zOffs * 16 + z;

            if (level->m_worldBorderEnabled &&
                (abs(worldX) >= Level::MAX_LEVEL_SIZE - 1 ||
                 abs(worldZ) >= Level::MAX_LEVEL_SIZE - 1))
            {
...
{
	EnterCriticalSection(&m_csRandom);
...
//    levelChunk->recalcHeightmap();		// 4J - removed & moved into its own method	
	...
	levelChunk->setGenStage(ChunkGenStage::Features);
	XPhysicalFree(tileData);

	LeaveCriticalSection(&m_csRandom);
...
{
	lc->recalcHeightmap();
	lc->setGenStage(ChunkGenStage::Complete);
}
...
// Staged generation: Stage 1 - Terrain shape only (noise-based blocks)
void RandomLevelSource::generateTerrain(LevelChunk* chunk, int xOffs, int zOffs, byteArray blocks)
{
	prepareHeights(xOffs, zOffs, blocks);
	...
	chunk->setGenStage(ChunkGenStage::Terrain);
}

// Staged generation: Stage 2 - Caves and canyons carved
void RandomLevelSource::generateCaves(LevelChunk* chunk, int xOffs, int zOffs, byteArray blocks)
{
	caveFeature->apply(this, level, xOffs, zOffs, blocks);
	canyonFeature->apply(this, level, xOffs, zOffs, blocks);

	chunk->setGenStage(ChunkGenStage::Caves);
}

// Staged generation: Stage 3 - Structures and features placed
void RandomLevelSource::generateFeatures(LevelChunk* chunk, int xOffs, int zOffs, byteArray blocks)
{
	if (generateStructures)
	{
		mineShaftFeature->apply(this, level, xOffs, zOffs, blocks);
	...
	chunk->setGenStage(ChunkGenStage::Features);
}
