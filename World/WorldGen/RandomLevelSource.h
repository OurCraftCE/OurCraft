...
private:
	...
	CRITICAL_SECTION m_csRandom;  // Protects random during getChunk/buildSurfaces
...
private:
	...
	floatArray pows;
	bool m_ownsCS;  // Whether this instance owns the critical section (false for thread-local copies)
...
public:
	// Staged generation methods - allow incremental chunk building
	void generateTerrain(LevelChunk* chunk, int xOffs, int zOffs, byteArray blocks);
	void generateCaves(LevelChunk* chunk, int xOffs, int zOffs, byteArray blocks);
	void generateFeatures(LevelChunk* chunk, int xOffs, int zOffs, byteArray blocks);
