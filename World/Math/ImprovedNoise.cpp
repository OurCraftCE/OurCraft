...
void ImprovedNoise::addSimd(doubleArray buffer, double _x, double _y, double _z, int xSize, int ySize, int zSize, double xs, double ys, double zs, double pow)
{
	if (SimdNoise::HasSSE2() || SimdNoise::HasAVX2())
	{
		SimdNoise::perlin_batch(
			buffer.data,
			_x, _y, _z,
			xSize, ySize, zSize,
			xs, ys, zs,
	...
	else
	{
		...
		add(buffer, _x, _y, _z, xSize, ySize, zSize, xs, ys, zs, pow);
	}
}
