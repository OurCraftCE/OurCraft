...
public:
	...
	// SIMD-accelerated add (auto-dispatches based on CPU features)
	void addSimd(doubleArray buffer, double _x, double _y, double _z, int xSize, int ySize, int zSize, double xs, double ys, double zs, double pow);

	// Expose permutation table for SIMD use
	const int* getPermTable() const { return p; }
};
