...
struct PlayerUID_Hash {
	size_t operator()(const PlayerUID& uid) const { return static_cast<size_t>(uid); }
};
struct INVITE_INFO
{
	...
	SessionID sessionID;
	PlayerUID hostXuid;
	WCHAR hostGamertag[XUSER_NAME_SIZE];
};
...
inline void *XPhysicalAlloc(SIZE_T a, ULONG_PTR  b, ULONG_PTR c, DWORD d) { return malloc(a); }
...
typedef struct _XMARKETPLACE_CONTENTOFFER_INFO
{
    ...
    DWORD dwPointsPrice;
    // Additional fields for Durango/Xbox One marketplace compatibility
    WCHAR wszProductID[256];
    WCHAR wszCurrencyPrice[64];
...
// Alias for Durango/Xbox One marketplace code that uses this name
...
#define XUserResetStatsViewAllUsers(...) ((DWORD)0)
