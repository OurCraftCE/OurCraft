...
void PlayerList::validatePlayerSpawnPosition(shared_ptr<ServerPlayer> player)
{
	...
	double targetX = 0;
	if(player->x < 0) targetX = Mth::ceil((float)player->x) - 0.5;
	else targetX = Mth::floor((float)player->x) + 0.5;

	double targetY = floor(player->y);

	double targetZ = 0;
	if(player->z < 0) targetZ = Mth::ceil((float)player->z) - 0.5;
	else targetZ = Mth::floor((float)player->z) + 0.5;
