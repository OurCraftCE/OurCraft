...
#include "net.minecraft.network.h"
#include "pos.h"
...
#endif
	{
        connection->send( shared_ptr<PreLoginPacket>( new PreLoginPacket(L"-", ugcXuids, ugcXuidCount, ugcFriendsOnlyBits, server->m_ugcPlayersVersion,szUniqueMapName,app.GetGameHostOption(eGameHostOption_All),(BYTE)hostIndex, server->m_texturePackId) ) );
    }
}
