...
#include "..\World\Level\BiomeSource.h"
...
int MinecraftServer::runPostUpdate(void* lpParam)
{
	...
	EnterCriticalSection(&server->m_postProcessCS);
	int maxRequests = (int)server->m_postProcessRequests.size();
...
void MinecraftServer::postProcessTerminate(ProgressRenderer *mcprogress)
{
	...
	do
	{
		...
		if( status == WAIT_TIMEOUT )
		{
			...
			{
				mcprogress->progressStagePercentage((int)((postProcessItemCount - postProcessItemRemaining) * 100 / postProcessItemCount));
			}
...
#else
		ConsoleSaveFile* pSave = new ConsoleSaveFileOriginal( initData->saveData->saveName, initData->saveData->data, (DWORD)initData->saveData->fileSize, false, initData->savePlatform );
#endif
...
//		m_lastSentDifficulty = pMinecraft->options->difficulty;
	...
	int i = 0;
    for (int i = 0; i < (int)levels.length ; i++)
	{
...
//					if (i == 0 || settings->getBoolean(L"allow-nether", true))		// 4J removed - we always have nether
			...
			for(int i=0;i<XUSER_MAX_COUNT;i++)
			{
				...
				case eXuiServerAction_DropItem:
					...
					player->drop( shared_ptr<ItemInstance>( new ItemInstance((int)id, 1, 0 ) ) );
					}
					break;
...
void MinecraftServer::cycleSlowQueueIndex()
{
	...
	do
	{
		...
		if( startingIndex >= (int)currentPlayerCount ) startingIndex = 0;
