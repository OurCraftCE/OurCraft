...
#include "Render/ProgressRenderer.h"
...
#include "Screens/InBedChatScreen.h"
#include "UI/AchievementPopup.h"
#include "Input/Input.h"
#include "Render/FrustumCuller.h"
#include "Render/Camera.h"
#include "Input/KeyboardMouseInput.h"

#include "..\World\Entities\MobEffect.h"
#include "..\World\Math\Difficulty.h"
#include "..\World\FwdHeaders/net.minecraft.world.level.h"
#include "..\World\FwdHeaders/net.minecraft.world.entity.h"
#include "..\World\FwdHeaders/net.minecraft.world.entity.player.h"
#include "..\World\FwdHeaders/net.minecraft.world.entity.item.h"
#include "..\World\FwdHeaders/net.minecraft.world.phys.h"
#include "..\World\IO\File.h"
#include "..\World\FwdHeaders/net.minecraft.world.level.storage.h"
#include "..\World\FwdHeaders/net.minecraft.h"
#include "..\World\FwdHeaders/net.minecraft.stats.h"
#include "..\World\IO\System.h"
#include "..\World\IO\ByteBuffer.h"
...
#include "..\World\WorldGen\HellRandomLevelSource.h"
#include "..\World\FwdHeaders/net.minecraft.world.entity.animal.h"
#include "..\World\FwdHeaders/net.minecraft.world.entity.monster.h"
#include "..\World\WorldGen\StrongholdFeature.h"
#include "..\World\Math\IntCache.h"
#include "..\World\Entities\Villager.h"
#include "..\World\Network\EntityIO.h"
#include "..\World\Level\SparseLightStorage.h"
#include "..\World\Level\SparseDataStorage.h"
#include "Textures/TextureManager.h"
#include "Common\UI\IUIScene_CreativeMenu.h"
#include "Common\UI\UIFontData.h"
#include "Textures/DLCTexturePack.h"
#include "Common\Discord\DiscordManager.h"
#include "Common\UI\UICursor.h"
#include "Screens/AbstractContainerScreen.h"
#include "ModLoader\ModLoader.h"
#include "ModLoader\ModAPIImpl.h"
#include "Windows64\Win64SavePaths.h"
#include "MinecraftServer.h"
#include "..\World\Level\ChunkSource.h"
#include "Platform\UIScreenManager.h"
#include "Platform\StorageManager.h"

// Bridge functions for UIScreenManager (which can't include Minecraft.h directly)
void UIScreenManager_AutoStartWorld(Minecraft* mc) { mc->autoStartWorld(); }
void UIScreenManager_StopGame(Minecraft* mc) { mc->stop(); }
void UIScreenManager_PerformSave(Minecraft* mc) { mc->performAutoSave(); }
void UIScreenManager_ReleaseLevel(Minecraft* mc) { mc->releaseLevel(-1); }
void UIScreenManager_Respawn(Minecraft* mc) {
	if (mc->player) mc->player->respawn();
}
void UIScreenManager_CreateWorld(Minecraft* mc, const char* name, int seed, int gameMode, int difficulty, int levelType, bool structures, bool bonusChest) {
	// Same as autoStartWorld but with custom settings
	mc->autoStartWorld();
}
void UIScreenManager_LoadWorld(Minecraft* mc, int worldIndex) {
	// For now, just start a new world (save loading needs more integration)
	mc->autoStartWorld();
}
void UIScreenManager_DeleteWorld(Minecraft* mc, int worldIndex) {
	// Get save info and delete via StorageManager
	extern CStorageManager StorageManager;
	PSAVE_DETAILS details = StorageManager.ReturnSavesInfo();
	if (details && worldIndex >= 0 && worldIndex < details->iSaveC) {
		StorageManager.DeleteSaveData(&details->SaveInfoA[worldIndex], nullptr, nullptr);
	}
}
int UIScreenManager_GetWorldCount(Minecraft* mc) {
	extern CStorageManager StorageManager;
	// Trigger enumeration
	StorageManager.GetSavesInfo(0, nullptr, nullptr, nullptr);
	PSAVE_DETAILS details = StorageManager.ReturnSavesInfo();
	return details ? details->iSaveC : 0;
}
const char* UIScreenManager_GetWorldName(Minecraft* mc, int index) {
	extern CStorageManager StorageManager;
	PSAVE_DETAILS details = StorageManager.ReturnSavesInfo();
	if (details && index >= 0 && index < details->iSaveC) {
		return details->SaveInfoA[index].UTF8SaveTitle;
	}
	return "Unknown";
}

int UIScreenManager_GetUITheme(Minecraft* mc) {
	if (mc && mc->options) return mc->options->uiTheme;
	return 0;
}

void UIScreenManager_CycleTheme(Minecraft* mc) {
	if (mc && mc->options) {
		mc->options->uiTheme = (mc->options->uiTheme + 1) % 3;
		mc->options->save();
	}
}

float UIScreenManager_GetMusicVolume(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->music : 1.0f;
}
float UIScreenManager_GetSoundVolume(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->sound : 1.0f;
}
void UIScreenManager_SetMusicVolume(Minecraft* mc, float vol) {
	if (mc && mc->options) mc->options->set(Options::Option::MUSIC, vol);
}
void UIScreenManager_SetSoundVolume(Minecraft* mc, float vol) {
	if (mc && mc->options) mc->options->set(Options::Option::SOUND, vol);
}
void UIScreenManager_SaveOptions(Minecraft* mc) {
	if (mc && mc->options) mc->options->save();
}

bool UIScreenManager_GetEulaAccepted(Minecraft* mc) {
	if (mc && mc->options) return mc->options->eulaAccepted;
	return false;
}

void UIScreenManager_SetEulaAccepted(Minecraft* mc) {
	if (mc && mc->options) {
		mc->options->eulaAccepted = true;
		mc->options->save();
	}
}

void UIScreenManager_SetLanguage(Minecraft* mc, const char* lang) {
	if (!mc || !mc->options || !lang) return;
	mc->options->language = lang;
	Language::getInstance()->Load(lang);
	mc->options->save();
}

const char* UIScreenManager_GetLanguage(Minecraft* mc) {
	if (!mc || !mc->options) return "en_US";
	static char buf[32];
	strncpy(buf, mc->options->language.c_str(), sizeof(buf) - 1);
	buf[sizeof(buf) - 1] = '\0';
	return buf;
}

// Video settings bridge functions
const char* UIScreenManager_GetVideoOption(Minecraft* mc, int optionId) {
	if (!mc || !mc->options) return "";
	const Options::Option* opt = Options::Option::getItem(optionId);
	if (!opt) return "";
	static char buf[128];
	std::wstring msg = mc->options->getMessage(opt);
	wcstombs(buf, msg.c_str(), sizeof(buf) - 1);
	buf[sizeof(buf) - 1] = '\0';
	return buf;
}
void UIScreenManager_CycleVideoOption(Minecraft* mc, int optionId) {
	if (!mc || !mc->options) return;
	const Options::Option* opt = Options::Option::getItem(optionId);
	if (opt) mc->options->toggle(opt, 1);
}
float UIScreenManager_GetFOV(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->fov : 0.0f;
}
void UIScreenManager_SetFOV(Minecraft* mc, float val) {
	if (mc && mc->options) mc->options->set(Options::Option::FOV, val);
}
float UIScreenManager_GetGamma(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->gamma : 0.0f;
}
void UIScreenManager_SetGamma(Minecraft* mc, float val) {
	if (mc && mc->options) mc->options->set(Options::Option::GAMMA, val);
}
int UIScreenManager_GetWindowMode(Minecraft* mc) {
	if (!mc || !mc->options) return 0;
	if (mc->options->windowFullscreen) return 1;
	if (mc->options->borderlessFullscreen) return 2;
	return 0;
}
void UIScreenManager_CycleWindowMode(Minecraft* mc) {
	if (!mc || !mc->options) return;
	int mode = UIScreenManager_GetWindowMode(mc);
	mode = (mode + 1) % 3;
	mc->options->windowFullscreen = (mode == 1);
	mc->options->borderlessFullscreen = (mode == 2);
}
const char* UIScreenManager_GetWindowModeStr(Minecraft* mc) {
	int mode = UIScreenManager_GetWindowMode(mc);
	switch (mode) {
...
int UIScreenManager_GetTargetFPS(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->targetFPS : 0;
}
void UIScreenManager_CycleTargetFPS(Minecraft* mc) {
	if (!mc || !mc->options) return;
	int fps = mc->options->targetFPS;
	if (fps == 0) fps = 30;
	else if (fps == 30) fps = 60;
	else if (fps == 60) fps = 120;
	else if (fps == 120) fps = 240;
	else fps = 0;
	mc->options->targetFPS = fps;
}
const char* UIScreenManager_GetTargetFPSStr(Minecraft* mc) {
	int fps = UIScreenManager_GetTargetFPS(mc);
	if (fps == 0) return "Unlimited";
	static char fpsBuf[16];
	snprintf(fpsBuf, sizeof(fpsBuf), "%d", fps);
	return fpsBuf;
}

// Controls settings bridge functions
void UIScreenManager_SetKeyBinding(Minecraft* mc, int index, int vkCode) {
	if (mc && mc->options && index >= 0 && index < Options::keyMappings_length) {
		mc->options->setKey(index, vkCode);
	}
}
const char* UIScreenManager_GetKeyName(Minecraft* mc, int index) {
	if (!mc || !mc->options || index < 0 || index >= Options::keyMappings_length) return "?";
	static char buf[64];
	int key = mc->options->keyMappings[index]->key;
	const wchar_t* name = GetVKDisplayName(key);
	wcstombs(buf, name, sizeof(buf) - 1);
	buf[sizeof(buf) - 1] = '\0';
	return buf;
}
float UIScreenManager_GetSensitivity(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->mouseSensitivity : 0.5f;
}
void UIScreenManager_SetSensitivity(Minecraft* mc, float val) {
	if (mc && mc->options) {
		mc->options->mouseSensitivity = val;
		mc->options->sensitivity = val;
	}
}
bool UIScreenManager_GetInvertY(Minecraft* mc) {
	return (mc && mc->options) ? mc->options->invertYMousePC : false;
}
void UIScreenManager_ToggleInvertY(Minecraft* mc) {
	if (mc && mc->options) {
		mc->options->invertYMousePC = !mc->options->invertYMousePC;
		mc->options->invertYMouse = mc->options->invertYMousePC;
		mc->options->save();
	}
}

void UIScreenManager_SendChat(Minecraft* mc, const char* msg) {
	// For now just log - chat integration needs local player
	printf("[Chat] %s\n", msg);
}

// HUD bridge functions for HUDOverlay (Platform code can't include Minecraft.h)
extern "C" {
    int HUD_GetPlayerHealth() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->getHealth();
        return 0;
    }
    int HUD_GetPlayerMaxHealth() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->getMaxHealth();
        return 20;
    }
    int HUD_GetPlayerFood() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->getFoodData()->getFoodLevel();
        return 20;
    }
    int HUD_GetPlayerActiveSlot() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->inventory->selected;
        return 0;
    }
    float HUD_GetPlayerXPProgress() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->experienceProgress;
        return 0.0f;
    }
    int HUD_GetPlayerXPLevel() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->experienceLevel;
        return 0;
    }
    int HUD_GetPlayerArmor() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->getArmorValue();
        return 0;
    }
    int HUD_GetPlayerAir() {
        Minecraft* mc = Minecraft::GetInstance();
        if (mc && mc->player) return mc->player->getAirSupply();
        return 300;
    }
    bool HUD_HasPlayer() {
        Minecraft* mc = Minecraft::GetInstance();
    ...
    bool HUD_IsInGame() {
        Minecraft* mc = Minecraft::GetInstance();
        return mc && mc->level != nullptr;
    }
}

// Inventory/Crafting/Chat bridge functions for RmlUi overlays
#include "..\World\Container\AbstractContainerMenu.h"
#include "..\World\Container\Slot.h"
extern "C" {
    int INV_GetItemId(int slot) {
        Minecraft* mc = Minecraft::GetInstance();
        if (!mc || !mc->player || !mc->player->inventory) return 0;
        if (slot >= 0 && slot < 36) {
            auto item = mc->player->inventory->getItem(slot);
            return item ? item->id : 0;
        }
        if (slot >= 36 && slot <= 39) {
            auto item = mc->player->inventory->getArmor(slot - 36);
            return item ? item->id : 0;
        }
        return 0;
    }
    ...
    int CRAFT_GetSlotItemId(int slot) {
        Minecraft* mc = Minecraft::GetInstance();
        if (!mc || !mc->player || !mc->player->containerMenu) return 0;
    ...
    void CHAT_SendMessage(const char* message) {
        Minecraft* mc = Minecraft::GetInstance();
        if (!mc || !mc->player || !message) return;
        // Convert to wstring and send as chat
        std::wstring wmsg;
        while (*message) { wmsg += (wchar_t)(unsigned char)*message++; }
        mc->player->sendMessage(wmsg);
    }

    // ========== Container Menu Bridges ==========
    void CONTAINER_SlotClicked(int slot, int button, bool shift) {
        // stub — full impl needs AbstractContainerMenu click integration
        (void)slot; (void)button; (void)shift;
    }
    ...
    int CONTAINER_GetSlotCount() {
        Minecraft* mc = Minecraft::GetInstance();
        if (!mc || !mc->player || !mc->player->containerMenu) return 0;
        return (int)mc->player->containerMenu->slots->size();
    }
    const char* CONTAINER_GetItemName(int itemId) {
        // stub — item name lookup requires Item registry integration
        (void)itemId;
        return "Item";
    }
}
...
void Minecraft::init()
{
	...
	// Use Win64SavePaths for world saves directory on PC
	if (Win64SavePaths::IsInitialized())
		levelSource = new McRegionLevelStorageSource(File(Win64SavePaths::GetWorldSavesDir()));
	else
	...
	*/
	...
	{
		// Show RmlUi title screen instead of bypassing to autoStartWorld()
		UIScreenManager::Get().Init(this);
		UIScreenManager::Get().ShowScreen(UIScreenManager::SCREEN_TITLE);
	}
	progressRenderer = new ProgressRenderer(this);

	RenderManager.CBuffLockStaticCreations();

	// Initialize the mod loader - scan and load mods from "Mods" directory
	{
		File modsDir(workingDirectory, L"Mods");
		std::string modsDirPath;
		std::wstring wpath = modsDir.getPath();
		for (size_t i = 0; i < wpath.length(); i++)
			modsDirPath += (char)wpath[i];
		ModLoader::Get().Initialize(modsDirPath);
		ModLoader::Get().LoadAllMods();
		ModLoader::Get().InitializeAllMods();
	}
}

void Minecraft::autoStartWorld()
{
	m_autoStarting = true;

	// Set default game options: Creative, Peaceful, Normal terrain
	...
	app.SetGameHostOption(eGameHostOption_LevelType, 0);  // Normal (not flat)
	...
	// Host a local singleplayer game
	g_NetworkManager.HostGame(1, false, false, 8, 0);
	g_NetworkManager.FakeLocalPlayerJoined();

	app.SetAutosaveTimerTime();

	// Launch the network/server thread (same as UIScene_FullscreenProgress does)
	C4JThread *thread = new C4JThread(&CGameNetworkManager::RunNetworkGameThreadProc, (void *)param, "AutoStart", 256 * 1024);
	thread->SetProcessor(CPU_CORE_UI_SCENE);
	thread->Run();
}
...
{
	if (workDir.getPath().empty())
	{
		// Use Win64SavePaths base directory if available
		if (Win64SavePaths::IsInitialized())
			workDir = File(Win64SavePaths::GetBaseDir());
		else
			workDir = getWorkingDirectory(L"minecraft");
	}
	return workDir;
}
...
void Minecraft::setScreen(Screen *screen)
{
	...
	if (screen == NULL && level == NULL && !m_autoStarting)
	{
	...
	if (dynamic_cast<TitleScreen *>(screen)!=NULL)
	{
		...
		gui->clearMessages();
		DiscordManager::Get().SetPresence(L"In Menus", L"Main Menu");
	}
	...
	if (screen != NULL)
	{
		...
		// Set cursor style: crosshair for container/inventory, arrow for menus
		if (dynamic_cast<AbstractContainerScreen *>(screen) != NULL)
		{
			UICursor::SetStyle(UICursor::eCursor_Crosshair);
		}
		else
		{
			UICursor::SetStyle(UICursor::eCursor_Arrow);
		}
	}
	else
	{
		// Re-grab mouse when returning to gameplay in KBM mode
		if (g_KBMInput.IsKBMActive() && level != NULL)
		{
...
{
	// Shutdown all mods before anything else
	...
	if(running)
	{
		...
		//while (running)
		{
			...
			for (int i = 0; i < timer->ticks; i++)
			{
				...
				for( int l = 0; l < (int)levels.length; l++ )
				{
					if( levels[l] )
					{
...
void Minecraft::emergencySave()
{
	...
	// Save world data before clearing
	performSaveOnExit();
...
void Minecraft::performAutoSave()
{
	if (level == NULL || m_autoSaveInProgress)
		return;

	m_autoSaveInProgress = true;

	app.DebugPrintf("Auto-save starting...\n");

	// Save level data for all active dimensions
	for (unsigned int i = 0; i < levels.length; ++i)
	{
		if (levels[i] != NULL && levels[i]->getLevelStorage() != NULL && levels[i]->getLevelData() != NULL)
		{
			// Save level data
			levels[i]->getLevelStorage()->saveLevelData(levels[i]->getLevelData());

			// Save player data
			PlayerIO *playerIO = levels[i]->getLevelStorage()->getPlayerIO();
			if (playerIO != NULL)
			{
				for (int p = 0; p < XUSER_MAX_COUNT; ++p)
				{
					if (localplayers[p] != nullptr)
					{
						playerIO->save(localplayers[p]);
					}
				}
			}

			// Flush the save file (autosave=true)
			levels[i]->getLevelStorage()->flushSaveFile(true);
		}
	}
...
void Minecraft::performSaveOnExit()
{
	if (level == NULL)
		return;

	app.DebugPrintf("Save-on-exit starting...\n");
	...
	for (unsigned int i = 0; i < levels.length; ++i)
	{
		if (levels[i] != NULL && levels[i]->getLevelStorage() != NULL && levels[i]->getLevelData() != NULL)
		{
			...
			// Full flush (not autosave)
			levels[i]->getLevelStorage()->flushSaveFile(false);
		}
	}
	...
	app.DebugPrintf("Save-on-exit complete.\n");
}
...
void Minecraft::onWindowResize(int newWidth, int newHeight)
{
	if (newWidth <= 0 || newHeight <= 0) return;
	width_phys = newWidth;
	height_phys = newHeight;
	width = newWidth;
	height = newHeight;
...
void Minecraft::tick(bool bFirst, bool bUpdateTextures)
{
	...
	// Tick all loaded mods (deltaTime = 1/20 for 20Hz tick rate)
	if (bFirst && !pause)
	{
	...
	if( bFirst )
	{
		...
		// Periodically update Discord player count in multiplayer (every ~5 seconds)
		if (ticks % 100 == 0 && !g_NetworkManager.IsLocalGame())
		{
			int playerCount = g_NetworkManager.GetPlayerCount();
			DiscordManager::Get().UpdatePlayerCount(playerCount, MINECRAFT_NET_MAX_PLAYERS);
		}

		// Auto-save every AUTOSAVE_INTERVAL_TICKS (5 minutes)
		if (level != NULL && !pause)
		{
			m_autoSaveTickCounter++;
			if (m_autoSaveTickCounter >= AUTOSAVE_INTERVAL_TICKS)
			{
				performAutoSave();
				m_autoSaveTickCounter = 0;
			}
	...
	*/
...

	{
		...
		if (dbgFrames < 20 && g_KBMInput.IsMouseGrabbed())
		{
			app.DebugPrintf("LOOK-DBG frame=%d dx=%d dy=%d\n",
				dbgFrames, g_KBMInput.GetMouseDeltaX(), g_KBMInput.GetMouseDeltaY());
			dbgFrames++;
		}
	}
	...
	{
		static int dbgGrab = 0;
		if (dbgGrab < 30)
		{
			app.DebugPrintf("GRAB-DBG screen=%p menuDisp=%d grabbed=%d focused=%d kbm=%d iPad=%d\n",
				screen, ui.GetMenuDisplayed(iPad)?1:0, g_KBMInput.IsMouseGrabbed()?1:0,
				g_KBMInput.IsWindowFocused()?1:0, g_KBMInput.IsKBMActive()?1:0, iPad);
			dbgGrab++;
		}
	}
	...
	if (screen == NULL && !ui.GetMenuDisplayed(iPad) )
	{
		...
		*piAction=-1;
		...
		if (iPad == 0 && wheel == 0)
		{
			...
			if (mw != 0)
			{
				int notches = mw / WHEEL_DELTA;
				if (notches == 0) notches = (mw > 0) ? 1 : -1;
				int dir = (notches > 0) ? -1 : 1;
				int count = abs(notches);
				// Apply extra notches directly (first notch handled by wheel below)
				for (int n = 1; n < count; n++)
					player->inventory->swapPaint(dir);
				wheel = dir;
			}
		}
		if (wheel != 0)
		...
		else if( gameMode->isInputAllowed(MINECRAFT_ACTION_USE) )
		{
			...
			else
			{
				...
				if (useHeld)
				{
					...
					if(player->isSleeping() ) player->lastClickTick[1] = ticks + (int)(timer->ticksPerSecond * 2);
					if( firstClick || autoRepeat )
					{
						...
						// If the player has just exited a bed, then delay the time before a repeat key is allowed without releasing
						if(wasSleeping) player->lastClickTick[1] = ticks + (int)(timer->ticksPerSecond * 2);
...
void Minecraft::setLevel(MultiPlayerLevel *level, int message /*=-1*/, shared_ptr<Player> forceInsertPlayer /*=NULL*/, bool doForceStatsSave /*=true*/, bool bPrimaryPlayerSignedOut /*=false*/)
{
	...
	// Fire world unload event for existing levels when setting to NULL
	if (level == NULL)
	{
		for(unsigned int i = 0; i < levels.length; ++i)
		{
			if (levels[i] != NULL)
			{
				ModLoader::Get().NotifyWorldUnload(levels[i]);
				ModEvent evt;
				evt.type = MOD_EVENT_WORLD_UNLOAD;
				evt.data.worldEvt.world = (ModWorld)levels[i];
				ModAPIImpl::Get().FireEvent(&evt);
			}
		}
	}
	...
	if (level != NULL)
	{
		...
		// Update Discord rich presence for gameplay
		{
			int dimId = (level->dimension) ? level->dimension->id : 0;
			DiscordManager::Get().UpdateDimensionPresence(dimId,
				level->getLevelData()->getLevelName().c_str());

			// Set game mode
			GameType* gt = level->getLevelData()->getGameType();
			if (gt)
			{
				DiscordManager::Get().UpdateGameModePresence(gt->getName().c_str());
			}

			// Set player count
			if (!g_NetworkManager.IsLocalGame() && g_NetworkManager.GetOnlinePlayerCount() > 1)
			{
				DiscordManager::Get().UpdatePlayerCount(
					g_NetworkManager.GetPlayerCount(), MINECRAFT_NET_MAX_PLAYERS);
			}
			else
			{
				DiscordManager::Get().UpdatePlayerCount(1, 0);
			}
		}

		// Fire world load event for mods
		ModLoader::Get().NotifyWorldLoad(level);
		{
			ModEvent evt;
			evt.type = MOD_EVENT_WORLD_LOAD;
			evt.data.worldEvt.world = (ModWorld)level;
			ModAPIImpl::Get().FireEvent(&evt);
		}

		// Fire player join event for primary player
		if (player != NULL)
		{
			ModEvent evt;
			evt.type = MOD_EVENT_PLAYER_JOIN;
			evt.data.playerEvt.player = (ModPlayer)player.get();
			evt.data.playerEvt.message = NULL;
			ModAPIImpl::Get().FireEvent(&evt);
		}
	}
	else
...
{
	// Update Discord presence to show loading state
	DiscordManager::Get().SetLoadingPresence(L"Building terrain");
...
void Minecraft::respawnPlayer(int iPad, int dimension, int newEntityId)
{
	...
	// Update Discord presence for dimension change
	DiscordManager::Get().UpdateDimensionPresence(dimension,
		level->getLevelData()->getLevelName().c_str());
...
// Helper: split a wstring by spaces
static vector<wstring> splitCommand(const wstring& str)
{
...
bool Minecraft::handleClientSideCommand(const wstring& chatMessage)
{
	...
	vector<wstring> args = splitCommand(chatMessage);
	...
	int iPad = localPlayerIdx;

	// /gamemode <0|1|2|s|c|a>
	if (cmd == L"/gamemode")
	{
		if (args.size() < 2) {
			gui->addMessage(L"Usage: /gamemode <0|1|2|s|c|a>", iPad);
			return true;
		}
		GameType *gt = NULL;
		wstring mode = args[1];
		if (mode == L"0" || mode == L"s" || mode == L"survival")
			gt = GameType::SURVIVAL;
		else if (mode == L"1" || mode == L"c" || mode == L"creative")
			gt = GameType::CREATIVE;
		else if (mode == L"2" || mode == L"a" || mode == L"adventure")
			gt = GameType::ADVENTURE;

		if (gt != NULL && player != nullptr) {
			player->setGameMode(gt);
			gui->addMessage(L"Set game mode to " + gt->getName(), iPad);
		} else {
			gui->addMessage(L"Unknown game mode: " + mode, iPad);
		}
		return true;
	}
	...
	{
		if (args.size() < 4) {
			gui->addMessage(L"Usage: /tp <x> <y> <z>", iPad);
			return true;
		}
		try {
			...
			if (player != nullptr) {
				player->moveTo(tx, ty, tz, player->yRot, player->xRot);
				wchar_t buf[128];
				swprintf(buf, 128, L"Teleported to %.1f %.1f %.1f", tx, ty, tz);
				gui->addMessage(buf, iPad);
			}
		} catch (...) {
			gui->addMessage(L"Invalid coordinates", iPad);
		}
		return true;
	}
	...
	{
		if (args.size() < 3) {
			gui->addMessage(L"Usage: /time <set|add> <value>", iPad);
			return true;
		}
		wstring sub = args[1];
		for (auto& c : sub) c = towlower(c);
		try {
			__int64 val = std::stoll(args[2]);
			if (level != NULL && level->getLevelData() != NULL) {
				if (sub == L"set") {
					level->getLevelData()->setTime(val);
					wchar_t buf[64];
					swprintf(buf, 64, L"Set time to %lld", val);
					gui->addMessage(buf, iPad);
				} else if (sub == L"add") {
					__int64 cur = level->getLevelData()->getTime();
					level->getLevelData()->setTime(cur + val);
					wchar_t buf[64];
					swprintf(buf, 64, L"Added %lld to time", val);
					gui->addMessage(buf, iPad);
				} else {
					gui->addMessage(L"Usage: /time <set|add> <value>", iPad);
				}
			...
			gui->addMessage(L"Invalid time value", iPad);
		}
		return true;
	}

	// /give <id> [count] [damage]
	if (cmd == L"/give")
	{
		if (args.size() < 2) {
			gui->addMessage(L"Usage: /give <id> [count] [damage]", iPad);
			return true;
		}
		try {
			...
			if (player != nullptr) {
				shared_ptr<ItemInstance> item = make_shared<ItemInstance>(itemId, count, damage);
				player->inventory->add(item);
				wchar_t buf[128];
				swprintf(buf, 128, L"Given [%d:%d] x%d", itemId, damage, count);
				gui->addMessage(buf, iPad);
			}
		} catch (...) {
			gui->addMessage(L"Invalid item parameters", iPad);
		}
		return true;
	}
	...
	if (cmd == L"/kill")
	{
		if (player != nullptr) {
			player->setHealth(0);
			gui->addMessage(L"Ouch. That looked like it hurt.", iPad);
		}
		return true;
	}

	// /toggledownfall
	if (cmd == L"/toggledownfall")
	{
		if (level != NULL) {
			level->toggleDownfall();
			gui->addMessage(L"Toggled downfall", iPad);
		}
		return true;
	}
	...
	{
		if (args.size() < 2) {
			gui->addMessage(L"Usage: /xp <amount>", iPad);
			return true;
		}
		// XP command stub - display message
		gui->addMessage(L"XP command not yet implemented", iPad);
		return true;
	}
	...
	if (cmd == L"/help")
	{
		gui->addMessage(L"Commands: /gamemode /tp /time /give /kill /toggledownfall /help", iPad);
		return true;
	}

	gui->addMessage(L"Unknown command: " + cmd + L". Type /help for help.", iPad);
	return true;
}
