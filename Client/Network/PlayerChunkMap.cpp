...
#ifdef _WINDOWS64
	// Limit light updates per chunk creation to avoid long stalls
	for (int lightIter = 0; lightIter < 64 && parent->getLevel()->updateLights(); lightIter++)
		;
#else
...
#ifdef _WINDOWS64
			// PC has more memory/CPU - process multiple chunks per tick
			static const int MAX_CHUNKS_PER_TICK = 8;
			int chunksProcessed = 1;
			while(chunksProcessed < MAX_CHUNKS_PER_TICK && addRequests.size() > 0)
			{
				minDistSq = -1;
				itNearest = addRequests.end();
				for( AUTO_VAR(it2, addRequests.begin()); it2 != addRequests.end(); it2++ )
				{
					if( it2->player == player )
					{
						int xm2 = ( it2->x  * 16 ) + 8;
						int zm2 = ( it2->z  * 16 ) + 8;
						int distSq2 = (xm2 - px) * (xm2 - px) + (zm2 - pz) * (zm2 - pz);
						if( ( minDistSq == -1 ) || ( distSq2 < minDistSq ) )
						{
							minDistSq = distSq2;
							itNearest = it2;
						}
...
bool PlayerChunkMap::chunkInBufferRange(int x, int z)
{
    int bufferedRadius = radius + UNLOAD_BUFFER;
    for (auto it = players.begin(); it != players.end(); ++it)
    {
        ...
        int xd = x - xc;
        int zd = z - zc;
        if (xd >= -bufferedRadius && xd <= bufferedRadius &&
            zd >= -bufferedRadius && zd <= bufferedRadius)
            return true;
    }
    return false;
}
