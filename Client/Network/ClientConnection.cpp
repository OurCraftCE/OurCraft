...
#include "../Screens/ReceivingLevelScreen.h"
#include "../Game/RemotePlayer.h"
#include "../Screens/DisconnectedScreen.h"
#include "../Particles/TakeAnimationParticle.h"
#include "../Particles/CritParticle.h"
#include "../Game/User.h"
...
#include "../../World/FwdHeaders/net.minecraft.world.entity.npc.h"
#include "../../World/FwdHeaders/net.minecraft.world.entity.item.h"
#include "../../World/FwdHeaders/net.minecraft.world.entity.projectile.h"
#include "../../World/FwdHeaders/net.minecraft.world.entity.global.h"
...
#include "../../World/FwdHeaders/net.minecraft.world.level.tile.entity.h"
#include "../../World/FwdHeaders/net.minecraft.world.item.h"
#include "../../World/FwdHeaders/net.minecraft.world.item.trading.h"
...
#include "../../World/FwdHeaders/net.minecraft.world.level.saveddata.h"
#include "../../World/FwdHeaders/net.minecraft.world.level.dimension.h"
#include "../../World/FwdHeaders/net.minecraft.world.effect.h"
#include "../../World/FwdHeaders/net.minecraft.world.food.h"
#include "../../World/Math\SharedConstants.h"
#include "../../World/Math\AABB.h"
#include "../../World/Math\Pos.h"
...
#ifdef _DURANGO
#include "../../World/Stats\DurangoStats.h"
#include "../../World/Stats\GenericStats.h"
#endif
...
{
	for (int i = 0; i < (int)packet->ids.length; i++)
	{
...
void ClientConnection::handleEntityActionAtPosition(shared_ptr<EntityActionAtPositionPacket> packet)
{
    ...
    if (packet->action == EntityActionAtPositionPacket::START_SLEEP)
	{
		...
		{
			TelemetryManager->RecordEnemyKilledOrOvercome(m_userIndex, 0, (int)player->y, 0, 0, 0, 0, eTelemetryInGame_UseBed);
		}
    }
}
...
//		minecraft->addPendingLocalConnection(m_userIndex, this);
		...
		if( packet->dimension == -1)
		{
			...
			DiscordManager::Get().SetLoadingPresence(L"Entering The Nether");
		}
		else if( oldDimension == -1)
		{
			param->stringId = IDS_PROGRESS_LEAVING_NETHER;
			DiscordManager::Get().SetLoadingPresence(L"Leaving The Nether");
		}
		else if( packet->dimension == 1)
		{
			param->stringId = IDS_PROGRESS_ENTERING_END;
			DiscordManager::Get().SetLoadingPresence(L"Entering The End");
		}
		else if( oldDimension == 1)
		{
			param->stringId = IDS_PROGRESS_LEAVING_END;
			DiscordManager::Get().SetLoadingPresence(L"Leaving The End");
		}
...
{
	unsigned int startingPrivileges = app.GetPlayerPrivileges((BYTE)packet->m_networkSmallId);

	INetworkPlayer *networkPlayer = g_NetworkManager.GetPlayerBySmallId((BYTE)packet->m_networkSmallId);
	...
	app.UpdatePlayerInfo((BYTE)packet->m_networkSmallId, packet->m_playerColourIndex, packet->m_playerPrivileges);
...
// 4J Stu - TU-1 hotfix
// Fix for #13191 - The host of a game can get a message informing them that the connection to the server has been lost
int ClientConnection::HostDisconnectReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
...
int ClientConnection::ExitGameAndSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
