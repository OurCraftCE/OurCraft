...
DWORD TexturePackRepository::hashPackName(const wstring &name)
{
	// Simple hash to generate a unique DWORD ID from pack name
	DWORD hash = 5381;
	for (size_t i = 0; i < name.length(); i++)
		hash = ((hash << 5) + hash) + (DWORD)name[i];
	return hash | 0x80000000; // Set high bit to avoid collision with DLC IDs
}

void TexturePackRepository::mergeVanillaOverlays(const wstring &resPackPath)
{
	// If we have a "vanilla" base folder, find all vanilla_X.XX.X overlay folders
	// and copy their textures on top to create a merged pack with the latest content
	File vanillaDir(resPackPath + L"\\vanilla");
	if (!vanillaDir.exists()) return;

	// Collect all vanilla_* overlay folder names
	vector<wstring> overlays;
	WIN32_FIND_DATAW findData;
	wstring searchPath = resPackPath + L"\\vanilla_*";
	HANDLE hFind = FindFirstFileW(searchPath.c_str(), &findData);
	if (hFind == INVALID_HANDLE_VALUE) return;

	do {
		if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			overlays.push_back(findData.cFileName);
	} while (FindNextFileW(hFind, &findData));
	FindClose(hFind);

	if (overlays.empty()) return;

	// Sort by version (lexicographic works for vanilla_1.XX.X format)
	sort(overlays.begin(), overlays.end());

	// Copy each overlay's textures/ on top of vanilla/textures/ in version order
	wstring vanillaTextures = resPackPath + L"\\vanilla\\textures";
	for (const auto &overlay : overlays)
	{
		wstring overlayTextures = resPackPath + L"\\" + overlay + L"\\textures";
		File overlayDir(overlayTextures);
		if (!overlayDir.exists()) continue;

		wstring cmd = L"xcopy \"" + overlayTextures + L"\" \"" + vanillaTextures + L"\" /E /Y /Q >nul 2>nul";
		char narrowCmd[1024];
		WideCharToMultiByte(CP_ACP, 0, cmd.c_str(), -1, narrowCmd, sizeof(narrowCmd), NULL, NULL);
		system(narrowCmd);
	}

	app.DebugPrintf("Merged %d vanilla overlays (latest: %ls)\n",
		(int)overlays.size(), overlays.back().c_str());
}

void TexturePackRepository::scanBedrockPacks()
{
	wstring resPackPath = L"Data/resource_packs";
	File resPackDir(resPackPath);
	if (!resPackDir.exists()) return;

	// Auto-merge vanilla_* overlays onto vanilla base for latest textures
	mergeVanillaOverlays(resPackPath);

	WIN32_FIND_DATAW findData;
	wstring searchPath = resPackPath + L"\\*";
	HANDLE hFind = FindFirstFileW(searchPath.c_str(), &findData);
	if (hFind == INVALID_HANDLE_VALUE) return;

	do {
		wstring entryName = findData.cFileName;
		if (entryName == L"." || entryName == L"..") continue;

		// Skip vanilla_* overlay folders — already merged into vanilla
		if (entryName.length() > 8 && entryName.substr(0, 8) == L"vanilla_") continue;

		wstring fullPath = resPackPath + L"\\" + entryName;

		if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			File manifest(fullPath + L"\\manifest.json");
			if (manifest.exists()) {
				DWORD id = hashPackName(entryName);
				if (cacheById.find(id) == cacheById.end()) {
					File *folder = new File(fullPath);
					BedrockFolderPack *pack = new BedrockFolderPack(id, entryName, folder, DEFAULT_TEXTURE_PACK);
					texturePacks->push_back(pack);
					cacheById[id] = pack;
					app.DebugPrintf("Found Bedrock folder pack: %ls (id=%u)\n", entryName.c_str(), id);
				}
			}
		} else {
			wstring lower = entryName;
			for (auto &c : lower) c = towlower(c);
			bool isMcpack = lower.length() > 7 && lower.substr(lower.length() - 7) == L".mcpack";
			bool isZip = lower.length() > 4 && lower.substr(lower.length() - 4) == L".zip";
			if (isMcpack || isZip) {
				DWORD id = hashPackName(entryName);
				if (cacheById.find(id) == cacheById.end()) {
					File *zipFile = new File(fullPath);
					BedrockZipPack *pack = new BedrockZipPack(id, zipFile, DEFAULT_TEXTURE_PACK);
					texturePacks->push_back(pack);
					cacheById[id] = pack;
					app.DebugPrintf("Found Bedrock zip pack: %ls (id=%u)\n", entryName.c_str(), id);
				}
			}
		}
	} while (FindNextFileW(hFind, &findData));
    ...
    // Auto-select the vanilla bedrock pack if nothing else is selected
    if (selected == DEFAULT_TEXTURE_PACK) {
        DWORD vanillaId = hashPackName(L"vanilla");
        auto it = cacheById.find(vanillaId);
        if (it != cacheById.end()) {
            selected = it->second;
            app.DebugPrintf("Auto-selected vanilla Bedrock pack\n");
        }
    }
}
