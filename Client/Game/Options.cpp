...
#include "../../World/IO\BufferedReader.h"
#include "../../World/IO\DataInputStream.h"
#include "../../World/IO\InputStreamReader.h"
#include "../../World/IO\FileInputStream.h"
#include "../../World/IO\FileOutputStream.h"
#include "../../World/IO\DataOutputStream.h"
...
void Options::init()
{
    ...
    // PC video settings defaults
    windowWidth = 1920;
    windowHeight = 1080;
    windowFullscreen = false;
    borderlessFullscreen = false;
    targetFPS = 0; // unlimited
    uiTheme = 0; // Bedrock default
    eulaAccepted = false;
    language = "en_US";
    Language::getInstance()->Load("en_US");

    // PC-specific input defaults
    mouseSensitivity = 0.5f;
    invertYMousePC = false;
    guiScalePC = 0;
...
Options::Options(Minecraft *minecraft, File workingDirectory)
{
	...
	// Use Win64SavePaths settings directory if available, otherwise fallback
	if (Win64SavePaths::IsInitialized())
		optionsFile = File(Win64SavePaths::GetSettingsDir(), L"options.txt");
	else
...
//            try {
                ...
                if (cmds[0] == L"windowWidth") windowWidth = _fromString<int>(cmds[1]);
                if (cmds[0] == L"windowHeight") windowHeight = _fromString<int>(cmds[1]);
                if (cmds[0] == L"windowFullscreen") windowFullscreen = cmds[1]==L"true";
                if (cmds[0] == L"borderlessFullscreen") borderlessFullscreen = cmds[1]==L"true";
                if (cmds[0] == L"targetFPS") targetFPS = _fromString<int>(cmds[1]);
                if (cmds[0] == L"mouseSensitivityPC") mouseSensitivity = readFloat(cmds[1]);
                if (cmds[0] == L"invertYMousePC") invertYMousePC = cmds[1]==L"true";
                if (cmds[0] == L"guiScalePC") guiScalePC = _fromString<int>(cmds[1]);
                if (cmds[0] == L"uiTheme") uiTheme = _fromString<int>(cmds[1]);
                if (cmds[0] == L"eulaAccepted") eulaAccepted = cmds[1]==L":true";
                if (cmds[0] == L"language") {
                    // cmds[1] starts with ':', strip it
                    std::wstring val = cmds[1];
                    if (!val.empty() && val[0] == L':') val = val.substr(1);
                    language = std::string(val.begin(), val.end());
                    Language::getInstance()->Load(language);
                }
...
//    try {
	...
	// Create backup of existing options file before writing
	if (optionsFile.exists())
	{
		File bakFile(optionsFile.getPath() + L".bak");
		CopyFileW(optionsFile.getPath().c_str(), bakFile.getPath().c_str(), FALSE);
	}
...
//        PrintWriter pw = new PrintWriter(new FileWriter(optionsFile));
        ...
        dos.writeChars(L"windowWidth:" + _toString<int>(windowWidth));
        dos.writeChars(L"windowHeight:" + _toString<int>(windowHeight));
        dos.writeChars(L"windowFullscreen:" + wstring(windowFullscreen ? L"true" : L"false"));
        dos.writeChars(L"borderlessFullscreen:" + wstring(borderlessFullscreen ? L"true" : L"false"));
        dos.writeChars(L"targetFPS:" + _toString<int>(targetFPS));
        dos.writeChars(L"mouseSensitivityPC:" + _toString<float>(mouseSensitivity));
        dos.writeChars(L"invertYMousePC:" + wstring(invertYMousePC ? L"true" : L"false"));
        dos.writeChars(L"guiScalePC:" + _toString<int>(guiScalePC));
        dos.writeChars(L"uiTheme:" + _toString<int>(uiTheme));
        dos.writeChars(L"eulaAccepted:" + wstring(eulaAccepted ? L"true" : L"false"));
        dos.writeChars(L"language:" + wstring(language.begin(), language.end()));
