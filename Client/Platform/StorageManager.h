...
public:
	...
	// Save transfer states (originally Sony/Xbox save transfer)
	enum eSaveTransferState
	{
		eSaveTransfer_Idle=0,
		eSaveTransfer_Busy,
	...
	// Save transfer file details (originally Sony save transfer struct)
	typedef struct
	{
		unsigned long ulFileLen;
		PBYTE pbData;
	}
	SAVETRANSFER_FILE_DETAILS, *PSAVETRANSFER_FILE_DETAILS;

	enum eOptionsCallback
	{
		eOptionsCallback_Idle=0,
		eOptionsCallback_Pending,
		eOptionsCallback_Success,
		eOptionsCallback_Fail,
		// Aliases used by game code
		eOptions_Callback_Idle = eOptionsCallback_Idle,
		eOptions_Callback_Pending = eOptionsCallback_Pending,
		eOptions_Callback_Success = eOptionsCallback_Success,
		eOptions_Callback_Fail = eOptionsCallback_Fail,
	...
	// Messages
	CStorageManager::EMessageResult			RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad=XUSER_INDEX_ANY,
		int( *Func)(LPVOID,int,const CStorageManager::EMessageResult)=NULL,LPVOID lpParam=NULL, C4JStringTable *pStringTable=NULL, WCHAR *pwchFormatString=NULL,DWORD dwFocusButton=0);


	CStorageManager::EMessageResult			GetMessageBoxResult();
	...
	CStorageManager::ESaveGameState	SaveSaveData(int( *Func)(LPVOID ,const bool),LPVOID lpParam);
	...
	CStorageManager::ESaveGameState	DoesSaveExist(bool *pbExists);
	...
	CStorageManager::ESaveGameState	GetSavesInfo(int iPad,int ( *Func)(LPVOID lpParam,SAVE_DETAILS *pSaveDetails,const bool),LPVOID lpParam,char *pszSavePackName);
	PSAVE_DETAILS				ReturnSavesInfo();
	void						ClearSavesInfo();	// Clears results
	CStorageManager::ESaveGameState	LoadSaveDataThumbnail(PSAVE_INFO pSaveInfo,int( *Func)(LPVOID lpParam,PBYTE pbThumbnail,DWORD dwThumbnailBytes), LPVOID lpParam, bool bForceLoad=false);	// Get the thumbnail for an individual save referenced by pSaveInfo
	CStorageManager::ESaveGameState	RenameSaveData(int iSaveIndex, uint16_t *pwchNewName, int( *Func)(LPVOID,bool), LPVOID lpParam);
	CStorageManager::ESaveGameState	GetSaveState();
	void						ReadFromProfile(int iQuadrant);
	...
	CStorageManager::ESaveGameState			LoadSaveData(PSAVE_INFO pSaveInfo,int( *Func)(LPVOID lpParam,const bool, const bool), LPVOID lpParam);
	CStorageManager::ESaveGameState		DeleteSaveData(PSAVE_INFO pSaveInfo,int( *Func)(LPVOID lpParam,const bool), LPVOID lpParam);

	// DLC
	void								RegisterMarketplaceCountsCallback(int ( *Func)(LPVOID lpParam, CStorageManager::DLC_TMS_DETAILS *, int), LPVOID lpParam );
	void								SetDLCPackageRoot(char *pszDLCRoot);
	CStorageManager::EDLCStatus				GetDLCOffers(int iPad,int( *Func)(LPVOID, int, DWORD, int),LPVOID lpParam, DWORD dwOfferTypesBitmask=XMARKETPLACE_OFFERING_TYPE_CONTENT);
	...
	DWORD								InstallOffer(int iOfferIDC, LPCWSTR wszProductId,int( *Func)(LPVOID, int, int),LPVOID lpParam, bool bTrial=false);
	DWORD								GetAvailableDLCCount( int iPad);

	CStorageManager::EDLCStatus				GetInstalledDLC(int iPad,int( *Func)(LPVOID, int, int),LPVOID lpParam);
	...
	DWORD								UnmountInstalledDLC(LPCWSTR wszMountDrive);
	void								GetMountedDLCFileList(const char* szMountDrive, std::vector<std::string>& fileList);
	std::string							GetMountedPath(std::string szMount);
	std::wstring						GetMountedPath(std::wstring wszMount);

	// Global title storage
	CStorageManager::ETMSStatus				ReadTMSFile(int iQuadrant,eGlobalStorage eStorageFacility,CStorageManager::eTMS_FileType eFileType,
	...
	CStorageManager::ETMSStatus				TMSPP_WriteFile(int iPad,CStorageManager::eGlobalStorage eStorageFacility,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,LPCWSTR pwchFilePath,PBYTE pBuffer,DWORD dwBufferSize,int( *Func)(LPVOID,int,int)=NULL,LPVOID lpParam=NULL, int iUserData=0);
	CStorageManager::ETMSStatus				TMSPP_WriteFile(int iPad,CStorageManager::eGlobalStorage eStorageFacility,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,CStorageManager::eTMS_UGCTYPE eUGCType,LPCSTR pchFilePath,PCHAR pchBuffer,DWORD dwBufferSize,int( *Func)(LPVOID,int,int)=NULL,LPVOID lpParam=NULL, int iUserData=0);
	CStorageManager::ETMSStatus				TMSPP_WriteFileWithProgress(int iPad,CStorageManager::eGlobalStorage eStorageFacility,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,CStorageManager::eTMS_UGCTYPE eUGCType,LPCSTR pchFilePath,PCHAR pchBuffer,DWORD dwBufferSize,int( *Func)(LPVOID,int,int)=NULL,LPVOID lpParam=NULL, int iUserData=0, int( *ProgressFunc)(LPVOID,float)=NULL, LPVOID lpProgressParam=NULL);
	CStorageManager::ETMSStatus				TMSPP_ReadFile(int iPad,CStorageManager::eGlobalStorage eStorageFacility,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,LPCSTR szFilename,int( *Func)(LPVOID,int,int,PTMSPP_FILEDATA, LPCSTR)=NULL,LPVOID lpParam=NULL, int iUserData=0);
	CStorageManager::ETMSStatus				TMSPP_ReadFile(int iPad,CStorageManager::eGlobalStorage eStorageFacility,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,LPCWSTR wszFilename,int( *Func)(LPVOID,int,int,LPVOID, WCHAR*)=NULL,LPVOID lpParam=NULL, int iUserData=0);
	CStorageManager::ETMSStatus				TMSPP_ReadFileList(int iPad,CStorageManager::eGlobalStorage eStorageFacility,int( *Func)(LPVOID,int,int,LPVOID,WCHAR*)=NULL,LPVOID lpParam=NULL, int iUserData=0);
	CStorageManager::ETMSStatus				TMSPP_DeleteFile(int iPad,LPCSTR szFilePath,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,int( *Func)(LPVOID,int,int)=NULL,LPVOID lpParam=NULL, int iUserData=0);
	CStorageManager::ETMSStatus				TMSPP_DeleteFile(int iPad,CStorageManager::eGlobalStorage eStorageFacility,CStorageManager::eTMS_FILETYPEVAL eFileTypeVal,LPCWSTR pwchFilePath,int( *Func)(LPVOID,int,int)=NULL,LPVOID lpParam=NULL, int iUserData=0);
...
// 	};
	...
	// Save transfer stubs (originally Sony save transfer)
	eSaveTransferState				SaveTransferGetDetails(int iPad, eGlobalStorage eStorageFacility, WCHAR *pwchFilename, int( *Func)(LPVOID, SAVETRANSFER_FILE_DETAILS *), LPVOID lpParam);
	eSaveTransferState				SaveTransferGetData(int iPad, eGlobalStorage eStorageFacility, WCHAR *pwchFilename, int( *Func)(LPVOID, SAVETRANSFER_FILE_DETAILS *), int( *ProgressFunc)(LPVOID, unsigned long), LPVOID lpParam, LPVOID lpProgressParam);
	eSaveTransferState				SaveTransferClearState();
	void							CancelSaveTransfer(int( *Func)(LPVOID), LPVOID lpParam);

	// Installed DLC lookup by product ID
