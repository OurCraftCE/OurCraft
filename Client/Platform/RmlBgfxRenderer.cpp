...
// WASM/Linux — GLSL ES variants (generated by wasm/build_shaders.sh)
#if (defined(__EMSCRIPTEN__) || defined(__linux__)) && __has_include("shaders/wasm/shader_rmlui_vs_glsl.h")
#  include "shaders/wasm/shader_rmlui_vs_glsl.h"
#  include "shaders/wasm/shader_rmlui_fs_glsl.h"
#  include "shaders/wasm/shader_rmlui_color_fs_glsl.h"
#  define RMLUI_HAS_GLSL_SHADERS 1
#endif
// macOS — Metal variants (generated by macos/build_shaders_metal.sh)
#if defined(__APPLE__) && __has_include("shaders/macos/shader_rmlui_vs_mtl.h")
#  include "shaders/macos/shader_rmlui_vs_mtl.h"
#  include "shaders/macos/shader_rmlui_fs_mtl.h"
#  include "shaders/macos/shader_rmlui_color_fs_mtl.h"
#  define RMLUI_HAS_METAL_SHADERS 1
#endif
...
bool RmlBgfxRenderer::LoadShaders()
{
	...
	uint32_t vsSize          = 0;
	const uint8_t* fsTexData = nullptr;
	uint32_t fsTexSize       = 0;
	const uint8_t* fsColData = nullptr;
	uint32_t fsColSize       = 0;
...
#ifdef RMLUI_HAS_GLSL_SHADERS
		vsData    = vs_rmlui_glsl;         vsSize    = sizeof(vs_rmlui_glsl);
		fsTexData = fs_rmlui_glsl;         fsTexSize = sizeof(fs_rmlui_glsl);
		fsColData = fs_rmlui_color_glsl;   fsColSize = sizeof(fs_rmlui_color_glsl);
		break;
#endif
		[[fallthrough]];
	case bgfx::RendererType::Metal:
#ifdef RMLUI_HAS_METAL_SHADERS
		vsData    = vs_rmlui_mtl;          vsSize    = sizeof(vs_rmlui_mtl);
		fsTexData = fs_rmlui_mtl;          fsTexSize = sizeof(fs_rmlui_mtl);
		fsColData = fs_rmlui_color_mtl;    fsColSize = sizeof(fs_rmlui_color_mtl);
		break;
#endif
		[[fallthrough]];
	...
	default:
		vsData    = vs_rmlui_d3d11;        vsSize    = sizeof(vs_rmlui_d3d11);
		fsTexData = fs_rmlui_d3d11;        fsTexSize = sizeof(fs_rmlui_d3d11);
		fsColData = fs_rmlui_color_d3d11;  fsColSize = sizeof(fs_rmlui_color_d3d11);
		break;
	}
...
void RmlBgfxRenderer::RenderGeometry(Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation, Rml::TextureHandle texture)
{
	...
	// premultiplied alpha blend, no depth test
...
void RmlBgfxRenderer::SetTransform(const Rml::Matrix4f* transform)
{
	...
	{
		m_hasTransform = true;
		// RmlUi Matrix4f is column-major; bgfx setTransform expects column-major
