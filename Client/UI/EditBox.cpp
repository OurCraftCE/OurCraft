...
void EditBox::keyPressed(wchar_t ch, int eventKey)
{
    ...
    if (ch == 22 || ch == 'v' || ch == 'V')
    {
        ...
        {
            wstring msg = Screen::getClipboard();
            int toAdd = (maxLength > 0 ? maxLength : 32) - (int)value.length();
            if (toAdd > (int)msg.length()) toAdd = (int)msg.length();
            if (toAdd > 0)
            {
                value += msg.substr(0, toAdd);
            }
            return; // Don't append the 'v' character
        }
#endif
    }
