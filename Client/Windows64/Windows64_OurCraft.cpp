// OurCraft.cpp : Defines the entry point for the application.
//
...
#include "..\..\World\Items\MapItem.h"
#include "..\..\World\Recipes\Recipes.h"
...
#include "..\..\World\Math\ThreadName.h"
...
#include "..\..\World\Level\OldChunkStorage.h"
#include "Network\WinsockNetLayer.h"
#include "..\Common\Discord\DiscordManager.h"
#include "Steam\SteamManager.h"

// Platform layer (SDL3 + bgfx)
#include "..\Platform\SDLWindow.h"
#include "..\Platform\SDLInput.h"
#include "..\Platform\SDLEventBridge.h"
// BgfxRenderer.h is now included via stdafx.h

// RmlUi integration
#include "..\Platform\RmlUIManager.h"

SDLWindow g_SDLWindow;
// g_SDLInput is defined in SDLInput.cpp
...
void SetFullscreen(bool fullscreen, bool borderless)
{
	...
	{
		// Windowed mode
		SetWindowLong(g_hWnd, GWL_STYLE, g_windowedStyle != 0 ? g_windowedStyle : WS_OVERLAPPEDWINDOW);
		if (g_windowedRect.right > g_windowedRect.left)
		{
		...
		{
			// First time going windowed, center on screen
...
{
	SetFullscreen(!g_isFullscreen, false);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KILLFOCUS:
		...
		{ extern void Windows64_RestoreGamma(); Windows64_RestoreGamma(); }
		break;
	...
	case WM_SETFOCUS:
		g_KBMInput.SetWindowFocused(true);
		{
			...
			float fVal = ((float)app.GetGameSettings(0, eGameSetting_Gamma)) * 327.68f;
			if (fVal > 0.0f) Windows64_UpdateGamma((unsigned short)fVal);
		}
		break;
	...
	case WM_CHAR:
		g_KBMInput.OnCharInput((wchar_t)wParam);
		break;
					   ...
					   _In_ int       nCmdShow)
{
	...
	// Initialize SDL3 window + bgfx renderer (replaces Win32 + D3D11)
	...
	{
		SDL_Log("Failed to init bgfx renderer");
		g_SDLWindow.Shutdown();
		return 1;
	}

	// Keep g_hWnd alive for any legacy code that still references it
	g_hWnd = (HWND)g_SDLWindow.GetNativeWindowHandle();
	hMyInst = hInstance;

	// Init RmlUi (must be after bgfx)
	RmlUIManager::Get().Init(g_iScreenWidth, g_iScreenHeight);

	// Init event bridge (routes SDL events to g_KBMInput + g_SDLInput)
	g_EventBridge.Init(&g_SDLWindow, &g_SDLInput, &g_KBMInput);
	...
	// TODO: RenderManager.Initialise - 4J render still used by game code, bgfx active underneath
	// RenderManager.Initialise(g_pd3dDevice, g_pSwapChain);
	...
	#define DISCORD_APP_ID 0LL
	DiscordManager::Get().Initialize(DISCORD_APP_ID);

	// Initialize Steam integration (App ID 480 / Spacewar for testing)
	if (!SteamManager::Initialize())
	{
		app.DebugPrintf("[Steam] Steam not available, online features disabled\n");
	}
	...
	// Initialize local DLC system - scan catalog and auto-load installed packs
	app.InitLocalDLC();
...
#pragma message(__LOC__"Need to define the _FINAL_BUILD before submission")
	...
	while (running)
	{
		...
		// Pump SDL events (replaces Win32 PeekMessage loop)
		if (!g_EventBridge.PumpEvents())
			break;

		// Update screen dimensions from SDL window
		...
		// RenderManager.Tick(); // TODO: bgfx handles its own state
		PIXEndNamedEvent();
		...
		// Tick Discord integration
		DiscordManager::Get().Tick();

		// Tick Steam integration
		SteamManager::Tick();
...
#endif
		// TODO: Iggy UI disabled — will be replaced by Bedrock JSON UI
		// {
		// 	Screen *scr = Minecraft::GetInstance()->screen;
		// 	bool hideIggy = (scr != NULL && scr->hidesIggyUI());
		// 	if(!hideIggy) ui.tick();
		// 	if(!hideIggy) ui.render();
		// }
#if 0
		...
		// Render software cursor on top of everything (Bedrock-style)
		UICursor::Render();

		// Present the frame.
		g_BgfxRenderer.Present();

		// ui.CheckMenuDisplayed(); // TODO: Iggy UI disabled, will be replaced by JSON UI
#if 0
	...
	if (pMinecraft && pMinecraft->soundEngine)
		pMinecraft->soundEngine->destroy();

	// Free resources
	// CleanupDevice(); // D3D11 cleanup no longer needed

	// Shutdown RmlUi (before bgfx)
