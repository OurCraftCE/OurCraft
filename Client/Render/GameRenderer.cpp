...
#include "..\World\Level\ChunkMemoryManager.h"
...
GameRenderer::GameRenderer(Minecraft *mc)
{
	...
	// Wire chunk memory callbacks to GameRenderer's deferred delete
	ChunkMemoryManager::addForDeleteSLS = [](SparseLightStorage* p) { GameRenderer::AddForDelete(p); };
	ChunkMemoryManager::addForDeleteCTS = [](CompressedTileStorage* p) { GameRenderer::AddForDelete(p); };
	ChunkMemoryManager::addForDeleteSDS = [](SparseDataStorage* p) { GameRenderer::AddForDelete(p); };
	ChunkMemoryManager::finishedReassigning = []() { GameRenderer::FinishedReassigning(); };
}
...
//			glRotatef(xRot - player->xRot, 1, 0, 0);
	...
	{
		glRotatef(player->xRotO + (player->xRot - player->xRotO) * a, 1, 0, 0);
		// All modes need +180 for the camera to face correctly.
		// Mode 2 (front-facing) uses the same rotation; the camera position
		// is already flipped by the +180 on yRot in the distance calc above.
		glRotatef(player->yRotO + (player->yRot - player->yRotO) * a + 180, 0, 1, 0);
	}
...
{
	// Use dynamic aspect ratio from Minecraft class (handles window resize)
	aspect = mc->getAspectRatio();
	if (aspect <= 0.0f) aspect = 16.0f / 9.0f; // fallback
		...
		for (int z = z0 - r; z <= z0 + r; z++)
		{
			...
			if (yy0 != yy1)
			{
				...
				if (level->getBiomeSource()->scaleTemp(temp, floor) >= 0.15f)
				{
                    ...
                    float br = 1;
                    t->offset((float)(-xo * 1), (float)(-yo * 1), (float)(-zo * 1));
                    t->tex2(level->getLightColor(x, yl, z, 0));
                    t->color(br, br, br, ((1 - dd * dd) * 0.5f + 0.5f) * rainLevel);
                    t->vertexUV((float)(x - xa + 0.5), (float)yy0, (float)(z - za + 0.5), 0 * s, (float)(yy0 * s / 4.0f + ra * s));
                    t->vertexUV((float)(x + xa + 0.5), (float)yy0, (float)(z + za + 0.5), 1 * s, (float)(yy0 * s / 4.0f + ra * s));
					t->color(br, br, br, 0.0f);														// 4J - added to soften the top visible edge of the rain
                    t->vertexUV((float)(x + xa + 0.5), (float)yy1, (float)(z + za + 0.5), 1 * s, (float)(yy1 * s / 4.0f + ra * s));
					t->vertexUV((float)(x - xa + 0.5), (float)yy1, (float)(z - za + 0.5), 0 * s, (float)(yy1 * s / 4.0f + ra * s));
					t->offset(0, 0, 0);
					t->end();
				}
				else
				{
                    ...
                    t->vertexUV((float)(x - xa + 0.5), (float)yy0, (float)(z - za + 0.5), 0 * s + uo, (float)(yy0 * s / 4.0f + ra * s + vo));
                    t->vertexUV((float)(x + xa + 0.5), (float)yy0, (float)(z + za + 0.5), 1 * s + uo, (float)(yy0 * s / 4.0f + ra * s + vo));
                    t->vertexUV((float)(x + xa + 0.5), (float)yy1, (float)(z + za + 0.5), 1 * s + uo, (float)(yy1 * s / 4.0f + ra * s + vo));
                    t->vertexUV((float)(x - xa + 0.5), (float)yy1, (float)(z - za + 0.5), 0 * s + uo, (float)(yy1 * s / 4.0f + ra * s + vo));
                    t->offset(0, 0, 0);
                }
			}
		}
