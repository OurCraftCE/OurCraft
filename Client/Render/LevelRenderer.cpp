...
#include "..\World\Math\BasicTypeContainers.h"
...
// so that we can render the "infinite" sea at the edges. Based on max view distance now that the world is infinite.
const int overworldSize = (PlayerChunkMap::MAX_VIEW_DISTANCE + 2) * 2; // 64
const int netherSize = (PlayerChunkMap::MAX_VIEW_DISTANCE + 2) * 2 / 8 + 2; // nether is 1/8 scale, plus margin
const int endSize = 18; // The End stays fixed size
...
void LevelRenderer::staticCtor()
{
	...
	{
		snprintf(threadName, sizeof(threadName), "Rebuild Chunk Thread %d\n", i);
