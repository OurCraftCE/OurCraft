...
#include "../Textures/HttpTexture.h"
#include "../Textures/MemTexture.h"
#include "..\World\IO\InputStream.h"
#include "..\World\IO\IntBuffer.h"
#include "..\World\IO\ByteBuffer.h"
#include "../Textures/TexturePack.h"
#include "../Game/Options.h"
#include "../Textures/MemTextureProcessor.h"
#include "../Textures/MobSkinMemTextureProcessor.h"
#include "../Textures/PreStitchedTextureMap.h"
#include "../Textures/TextureMap.h"
#include "../Textures/BedrockResourcePack.h"
#include "../Textures/BedrockFolderPack.h"
#include "../Textures/BedrockZipPack.h"
#include "../OurCraft.h"
#include "../Textures/StitchedTexture.h"
...
BgfxRenderer::eTextureFormat Textures::TEXTURE_FORMAT = BgfxRenderer::TEXTURE_FORMAT_RxGyBzAw;
...
//    pixels = MemoryTracker::createIntBuffer(2048 * 2048);	// 4J removed - now just creating this buffer when we need it
	...
	*/
	...
	terrain = new TextureMap(Icon::TYPE_TERRAIN, L"terrain", L"textures/blocks/", missingNo, true);
	items = new TextureMap(Icon::TYPE_ITEM, L"items", L"textures/items/", missingNo, true);

	// Wire up Bedrock resource pack if available
	TexturePack *selected = Minecraft::GetInstance()->skins->getSelected();
	BedrockResourcePack *bedrockPack = NULL;
	// Try BedrockFolderPack
	BedrockFolderPack *folderPack = dynamic_cast<BedrockFolderPack *>(selected);
	if (folderPack) bedrockPack = folderPack->getBedrockPack();
	// Try BedrockZipPack
	if (!bedrockPack) {
		BedrockZipPack *zipPack = dynamic_cast<BedrockZipPack *>(selected);
		if (zipPack) bedrockPack = zipPack->getBedrockPack();
	}
	if (bedrockPack) {
		terrain->setBedrockPack(bedrockPack);
		items->setBedrockPack(bedrockPack);
	}
...
int Textures::loadTexture(const wstring& resourceName)
{
	return loadTexture(TN_COUNT, resourceName);
}
...
void Textures::setTextureFormat(const wstring& resourceName)
{
	...
	{
		TEXTURE_FORMAT = BgfxRenderer::TEXTURE_FORMAT_RxGyBzAw;
	}
}
...
int Textures::getTexture(BufferedImage *img, BgfxRenderer::eTextureFormat format, bool mipmap)
{
    int id = MemoryTracker::genTextures();
...
int Textures::loadMemTexture(const wstring& url, const wstring& backup)
{
	...
	if(texture != NULL)
	{
		if (texture->loadedImage != NULL && !texture->isLoaded)
		{
			...
			if (texture->id < 0)
			{
				texture->id = getTexture(texture->loadedImage, BgfxRenderer::TEXTURE_FORMAT_RxGyBzAw, MIPMAP);
			}
			else
			{
				loadTexture(texture->loadedImage, texture->id);
...
void Textures::reloadAll()
{
	...
	// Re-wire Bedrock resource pack for the currently selected texture pack
	terrain->setBedrockPack(NULL);
	items->setBedrockPack(NULL);
	BedrockResourcePack *bedrockPack = NULL;
	BedrockFolderPack *folderPack = dynamic_cast<BedrockFolderPack *>(skin);
	if (folderPack) bedrockPack = folderPack->getBedrockPack();
	if (!bedrockPack) {
		BedrockZipPack *zipPack = dynamic_cast<BedrockZipPack *>(skin);
		if (zipPack) bedrockPack = zipPack->getBedrockPack();
	}
	if (bedrockPack) {
		terrain->setBedrockPack(bedrockPack);
