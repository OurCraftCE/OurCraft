...
void Input::tick(LocalPlayer *player)
{
	...
	{
		static int dbgInputFrames = 0;
		if (dbgInputFrames < 5)
		{
			app.DebugPrintf("INPUT-DBG frame=%d iPad=%d grabbed=%d lReset=%d rReset=%d\n",
				dbgInputFrames, iPad, g_KBMInput.IsMouseGrabbed()?1:0, lReset?1:0, rReset?1:0);
			dbgInputFrames++;
		}
	}
	...
	if (iPad == 0 && g_KBMInput.IsMouseGrabbed())
	{
		...
		static int dbgTurnFrames = 0;
		if (dbgTurnFrames < 10 && (mx != 0.0f || my != 0.0f))
		{
			app.DebugPrintf("TURN-DBG mx=%.1f my=%.1f turnX=%.1f turnY=%.1f sens=%.2f rReset=%d\n", mx, my, turnX, turnY, mouseSensitivity, rReset?1:0);
			dbgTurnFrames++;
		}
	}
#endif
