...
HRESULT CScene_TransferToXboxOne::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	if(StorageManager.TMSPP_InFileList(CStorageManager::eGlobalStorage_TitleUser,m_iPad,L"XboxOne/SlotList"))
	{
		// there is a slot list file with details of the saves
		CStorageManager::ETMSStatus status=StorageManager.TMSPP_ReadFile(m_iPad,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,"XboxOne/SlotList",&CScene_TransferToXboxOne::TMSPPSlotListReturned,this);
...
// TMSPPSlotListReturned callback
//----------------------------------------------------------------------------------
int CScene_TransferToXboxOne::TMSPPSlotListReturned(LPVOID pParam,int iPad,int iUserData,CStorageManager::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
	CScene_TransferToXboxOne* pClass = (CScene_TransferToXboxOne *) pParam;
...
int CScene_TransferToXboxOne::UploadSaveForXboxOneThreadProc( LPVOID lpParameter )
{
	...
	CStorageManager::ETMSStatus eStatus=StorageManager.TMSPP_WriteFile(pClass->m_iPad,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,CStorageManager::TMS_UGCTYPE_NONE,"XboxOne/SlotList",(PCHAR) pClass->m_pbSlotListFile, pClass->m_uiSlotListFileBytes,
		&CScene_TransferToXboxOne::TMSPPWriteReturned,lpParameter, 0);

	if(eStatus!=CStorageManager::ETMSStatus_WriteInProgress)
	{
		// failed
		pClass->m_bWaitingForWrite=false;
	}
	else
	{
		...
		CStorageManager::ELoadGameStatus eLoadStatus=StorageManager.LoadSaveData(&pClass->m_XContentData,CScene_TransferToXboxOne::LoadSaveDataReturned,lpParameter);
		...
		CStorageManager::ETMSStatus eStatus;
		...
		for(int i=0;i<(pClass->m_uiStorageLength/uiChunkSize)+1;i++)
		{
			...
			{
				eStatus=StorageManager.TMSPP_WriteFile(pClass->m_iPad,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,CStorageManager::TMS_UGCTYPE_NONE,szFilename,pchData, uiChunkSize,
					&CScene_TransferToXboxOne::TMSPPWriteReturned,lpParameter, 0);
			...
			else
			{
				...
				eStatus=StorageManager.TMSPP_WriteFile(pClass->m_iPad,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,CStorageManager::TMS_UGCTYPE_NONE,szFilename,pchData, uiBytesLeft,
					&CScene_TransferToXboxOne::TMSPPWriteReturned,lpParameter, 0);
			}
...
HRESULT CScene_TransferToXboxOne::OnKeyDown(XUIMessageInput* pInputData, BOOL& rfHandled)
{
	...
	case VK_PAD_X:
		...
		if(m_pbSlotListFile!=NULL)
		{
			...
			CStorageManager::ETMSStatus eStatus=StorageManager.TMSPP_DeleteFile(m_iPad,"XboxOne/SlotList",CStorageManager::TMS_FILETYPE_BINARY,&CScene_TransferToXboxOne::TMSPPDeleteReturned,this, 0);
