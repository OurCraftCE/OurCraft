...
DWORD DLCManager::checkForCorruptDLCAndAlert(bool showMessage /*= true*/)
{
	...
	if(corruptDLCCount > 0 && showMessage)
	{
		...
		if(corruptDLCCount == 1 && firstCorruptPack != NULL)
		{
			...
			CStorageManager::EMessageResult result = ui.RequestMessageBox( IDS_CORRUPT_DLC_TITLE, IDS_CORRUPT_DLC, uiIDA,1,ProfileManager.GetPrimaryPad(),NULL,NULL, app.GetStringTable(),wchFormat);

		}
		else
		{
			CStorageManager::EMessageResult result = ui.RequestMessageBox( IDS_CORRUPT_DLC_TITLE, IDS_CORRUPT_DLC_MULTIPLE, uiIDA,1,ProfileManager.GetPrimaryPad(),NULL,NULL, app.GetStringTable());
		}
	}
...
bool DLCManager::processDLCDataFile(DWORD &dwFilesProcessed, PBYTE pbData, DWORD dwLength, DLCPack *pack)
{
	...
	uiCurrentByte+=sizeof(int);
	CStorageManager::DLC_FILE_PARAM *pParams = (CStorageManager::DLC_FILE_PARAM *)&pbData[uiCurrentByte];
	//DWORD dwwchCount=0;
	for(unsigned int i=0;i<uiParameterCount;i++)
	{
		...
		uiCurrentByte+= sizeof(CStorageManager::DLC_FILE_PARAM)+(pParams->dwWchCount*sizeof(WCHAR));
		pParams = (CStorageManager::DLC_FILE_PARAM *)&pbData[uiCurrentByte];
	}
	//ulCurrentByte+=ulParameterCount * sizeof(CStorageManager::DLC_FILE_PARAM);

	unsigned int uiFileCount=*(unsigned int *)&pbData[uiCurrentByte];
	uiCurrentByte+=sizeof(int);
	CStorageManager::DLC_FILE_DETAILS *pFile = (CStorageManager::DLC_FILE_DETAILS *)&pbData[uiCurrentByte];

	DWORD dwTemp=uiCurrentByte;
	for(unsigned int i=0;i<uiFileCount;i++)
	{
		dwTemp+=sizeof(CStorageManager::DLC_FILE_DETAILS)+pFile->dwWchCount*sizeof(WCHAR);
		pFile = (CStorageManager::DLC_FILE_DETAILS *)&pbData[dwTemp];
	}
	PBYTE pbTemp=((PBYTE )pFile);//+ sizeof(CStorageManager::DLC_FILE_DETAILS)*ulFileCount;
	pFile = (CStorageManager::DLC_FILE_DETAILS *)&pbData[uiCurrentByte];
	...
	for(unsigned int i=0;i<uiFileCount;i++)
	{
		...
		pbTemp+=sizeof(int);
		pParams = (CStorageManager::DLC_FILE_PARAM *)pbTemp;
		for(unsigned int j=0;j<uiParameterCount;j++)
		{
			...
			pbTemp+=sizeof(CStorageManager::DLC_FILE_PARAM)+(sizeof(WCHAR)*pParams->dwWchCount);
			pParams = (CStorageManager::DLC_FILE_PARAM *)pbTemp;
		}
		//pbTemp+=ulParameterCount * sizeof(CStorageManager::DLC_FILE_PARAM);
		...
		uiCurrentByte+=sizeof(CStorageManager::DLC_FILE_DETAILS)+pFile->dwWchCount*sizeof(WCHAR);
