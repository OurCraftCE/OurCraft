...
#include "..\..\World\Network\DisconnectPacket.h"
...
public:
	...
	static const float fSafeZoneX; // 5% of screen width (legacy constant for 1280)
	static const float fSafeZoneY; // 5% of screen height (legacy constant for 720)
	static float getSafeZoneX(); // Dynamic 5% of actual screen width
	static float getSafeZoneY(); // Dynamic 5% of actual screen height
	...
	// Initialize the local DLC catalog and installer paths
	void InitLocalDLC();
	// Install a pack from catalog by ID, load it into DLCManager
	bool InstallLocalDLCPack(const wstring &packId);
	// Uninstall a pack by ID
	bool UninstallLocalDLCPack(const wstring &packId);
	...
	static int		DisplaySavingMessage(LPVOID pParam,const CStorageManager::ESavingMessage eMsg, int iPad);
	...
	bool			LoadCraftingTableMenu(int iPad, shared_ptr<LocalPlayer> player, int x, int y, int z);
	...
	static int		OptionsDataCallback(LPVOID pParam,int iPad,unsigned short usVersion,CStorageManager::eOptionsCallback eStatus);

	CStorageManager::eOptionsCallback GetOptionsCallbackStatus(int iPad);

	void			SetOptionsCallbackStatus(int iPad, CStorageManager::eOptionsCallback eStatus);
...
public:
	...
	static int PrimaryPlayerSignedOutReturned(void *pParam, int iPad, const CStorageManager::EMessageResult);
	static int EthernetDisconnectReturned(void *pParam, int iPad, const CStorageManager::EMessageResult);
	...
	//bool			UploadFileToGlobalStorage(int iQuadrant, CStorageManager::eGlobalStorage eStorageFacility, wstring *wsFile  );
	...
	//static int DLCReadCallback(LPVOID pParam,CStorageManager::DLC_FILE_DETAILS *pDLCData);
	...
	static int MarketplaceCountsCallback(LPVOID pParam,CStorageManager::DLC_TMS_DETAILS *,int iPad);
...
private:
	...
	unordered_map<PlayerUID, PBYTE, PlayerUID_Hash> m_GTS_Files;
...
private:
	...
	static int BannedLevelDialogReturned(void *pParam,int iPad,const CStorageManager::EMessageResult);
	static int TexturePackDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
...
private:
	...
	static int		UnlockFullExitReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		UnlockFullSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		UnlockFullInviteReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		TrialOverReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		ExitAndJoinFromInvite(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		ExitAndJoinFromInviteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		ExitAndJoinFromInviteAndSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		ExitAndJoinFromInviteDeclineSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
	static int		FatalErrorDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
...
public:
	...
	static int ExitGameFromRemoteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result);
private:
...
private:
	...
	static unordered_map<PlayerUID,MOJANG_DATA *, PlayerUID_Hash > MojangData;
...
public:
	...
	static int		TMSPPFileReturned(LPVOID pParam,int iPad,int iUserData,LPVOID pFileData, WCHAR *wchFilename);
...
private:
	...
	CStorageManager::eOptionsCallback m_eOptionsStatusA[XUSER_MAX_COUNT];
