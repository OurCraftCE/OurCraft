...
#include "..\Discord\DiscordXUIDMapper.h"
#include "..\..\Windows64\Steam\SteamManager.h"
#include "..\..\Windows64\Steam\SteamNetworkManager.h"
...
void CGameNetworkManager::Initialise()
{
	...
	// Create RakNet transport and adapter
	m_transport = new RakNetTransport();
	m_adapter = new NetworkAdapter(m_transport);
	Socket::SetNetworkAdapter(m_adapter);

	// Use Steam network manager when Steam is available, otherwise fall back to stub (LAN-only)
	if (SteamManager::IsInitialized())
	{
		s_pPlatformNetworkManager = new CSteamNetworkManager();
		app.DebugPrintf("[Steam] Using SteamNetworkManager for multiplayer\n");
	}
	else
	{
		s_pPlatformNetworkManager = new CPlatformNetworkManagerStub();
		app.DebugPrintf("Using stub network manager (LAN only)\n");
	}
...
void CGameNetworkManager::Terminate()
{
	...
	if (m_adapter) { delete m_adapter; m_adapter = nullptr; }
	if (m_transport) { m_transport->stop(); delete m_transport; m_transport = nullptr; }
}

void CGameNetworkManager::DoWork()
{
	// Pump RakNet data into Socket queues
	if (m_adapter)
		m_adapter->tick();
...
//		printf("Server ready to go!\n");
	...
	if( !g_NetworkManager.IsHost() )
	{
		...
		DiscordManager::Get().SetLoadingPresence(L"Connecting to server");
	}
	else
	{
...
void CGameNetworkManager::HostGame(int localUsersMask, bool bOnlineGame, bool bIsPrivate, unsigned char publicSlots, unsigned char privateSlots)
{
	...
	// Set Discord join secret so friends can join via Discord
	if (bOnlineGame && !bIsPrivate && DiscordManager::Get().IsConnected())
	{
		// Format: "MC|<hostIp>|<port>|<sessionId>"
		// For now use localhost placeholder - real implementation would use
		// the actual external IP and session ID from the platform network manager
		char secret[128];
		snprintf(secret, sizeof(secret), "MC|127.0.0.1|19132|%llu",
		         (unsigned long long)DiscordManager::Get().GetCurrentUserId());
		DiscordManager::Get().SetJoinSecret(secret);
	}
}
...
int	CGameNetworkManager::ExitAndJoinFromInviteThreadProc( void* lpParam )
{
	...
	IUIScene_PauseMenu::_ExitWorld(NULL);
...
int	CGameNetworkManager::MustSignInReturned_1(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
...
int CGameNetworkManager::ChangeSessionTypeThreadProc( void* lpParam )
{
	...
	if( g_NetworkManager.m_bFullSessionMessageOnNextSessionChange )
	{
		UINT uiIDA[1];
		uiIDA[0]=IDS_CONFIRM_OK;
		CStorageManager::EMessageResult result = ui.RequestMessageBox( IDS_PROGRESS_CONVERTING_TO_OFFLINE_GAME, IDS_IN_PARTY_SESSION_FULL, uiIDA,1,ProfileManager.GetPrimaryPad());
