...
void CPlatformNetworkManagerStub::DoWork()
{
	...
	RakNetTransport* transport = g_NetworkManager.GetTransport();
	NetworkAdapter* adapter = g_NetworkManager.GetAdapter();

	if (_iQNetStubState == QNET_STATE_GAME_PLAY && m_pIQNet->IsHost() && transport)
	{
		// Poll for new RakNet connections
		ConnectionId newConn;
		while (transport->pollNewConnection(newConn))
		{
			BYTE assignedSmallId;
			if (!m_freeSmallIds.empty())
			{
				assignedSmallId = m_freeSmallIds.back();
				m_freeSmallIds.pop_back();
			}
			else if (m_nextSmallId < MINECRAFT_NET_MAX_PLAYERS)
			{
				assignedSmallId = m_nextSmallId++;
			}
			else
			{
				app.DebugPrintf("RakNet: Server full, closing connection\n");
				transport->closeConnection(newConn);
				continue;
			}

			m_connIdToSmallId[newConn.id] = assignedSmallId;

			// Send smallId to client as first message
			transport->send(newConn, &assignedSmallId, 1, PacketPriority::High, PacketReliability::ReliableOrdered);

			app.DebugPrintf("RakNet: Client connected, assigned smallId=%d\n", assignedSmallId);
			...
			// Register the socket with the adapter for data routing
			INetworkPlayer *np = getNetworkPlayer(qnetPlayer);
			if (np && np->GetSocket() && adapter)
			{
				adapter->registerConnection(newConn, np->GetSocket(), np);
			}
		}

		// Poll for RakNet disconnections
		ConnectionId discConn;
		while (transport->pollDisconnection(discConn))
		{
			auto it = m_connIdToSmallId.find(discConn.id);
			if (it != m_connIdToSmallId.end())
			{
				BYTE disconnectedSmallId = it->second;
				...
				if (qnetPlayer != NULL && qnetPlayer->m_smallId == disconnectedSmallId)
				{
					...
					m_freeSmallIds.push_back(disconnectedSmallId);
					if (IQNet::s_playerCount > 1)
						IQNet::s_playerCount--;
				}
				if (adapter)
					adapter->removeConnection(discConn);
				m_connIdToSmallId.erase(it);
			}
		}

		// Check for game-side socket close requests
		for (int i = 1; i < MINECRAFT_NET_MAX_PLAYERS; i++)
		{
			...
			{
				for (auto &pair : m_connIdToSmallId)
				{
					if (pair.second == (BYTE)i)
					{
						ConnectionId cid;
						cid.id = pair.first;
						transport->closeConnection(cid);
						break;
					}
				}
			}
	...
	{
		// Check if RakNet connection to host was lost
		if (transport && !transport->isActive() && !g_NetworkManager.IsLeavingGame())
		{
...
bool CPlatformNetworkManagerStub::LeaveGame(bool bMigrateHost)
{
	...
	// Stop RakNet transport
	RakNetTransport* transport = g_NetworkManager.GetTransport();
	if (transport)
		transport->stop();

	m_connIdToSmallId.clear();
	m_freeSmallIds.clear();
	m_nextSmallId = 1;
	m_rakNetHostConnection = ConnectionId{};
...
void CPlatformNetworkManagerStub::HostGame(int localUsersMask, bool bOnlineGame, bool bIsPrivate, unsigned char publicSlots /*= MINECRAFT_NET_MAX_PLAYERS*/, unsigned char privateSlots /*= 0*/)
{
	...
	// Listen for connections via RakNet
	int port = WIN64_NET_DEFAULT_PORT;
	RakNetTransport* transport = g_NetworkManager.GetTransport();
	if (transport && !transport->isActive())
	{
		if (transport->listen(port, MINECRAFT_NET_MAX_PLAYERS))
			app.DebugPrintf("RakNet: Hosting on port %d\n", port);
		else
			app.DebugPrintf("RakNet: Failed to listen on port %d\n", port);
	}

	// Keep LAN advertising via WinsockNetLayer (UDP broadcast)
...
int CPlatformNetworkManagerStub::JoinGame(FriendSessionInfo *searchResult, int localUsersMask, int primaryUserIndex)
{
	...
	// Connect via RakNet
	RakNetTransport* transport = g_NetworkManager.GetTransport();
	if (!transport || !transport->connect(hostIP, hostPort))
	{
		app.DebugPrintf("RakNet: Failed to connect to %s:%d\n", hostIP, hostPort);
		return CGameNetworkManager::JOINGAME_FAIL_GENERAL;
	}

	// Wait for RakNet connection to establish (poll up to 5 seconds)
	for (int i = 0; i < 500; i++)
	{
		transport->tick();
		ConnectionId connId;
		if (transport->pollNewConnection(connId))
		{
			// Connected — register as the host connection
			NetworkAdapter* adapter = g_NetworkManager.GetAdapter();
			m_rakNetHostConnection = connId;
			break;
		}
		Sleep(10);
		if (i == 499)
		{
			app.DebugPrintf("RakNet: Connection timeout to %s:%d\n", hostIP, hostPort);
			transport->disconnect();
			return CGameNetworkManager::JOINGAME_FAIL_GENERAL;
		}
	}

	// Wait for smallId assignment from host (first byte of first message)
	BYTE localSmallId = 1;
	for (int i = 0; i < 500; i++)
	{
		transport->tick();
		IncomingPacket* pkt = transport->receive();
		if (pkt && pkt->length >= 1)
		{
			localSmallId = pkt->data[0];
			transport->deallocatePacket(pkt);
			break;
		}
		if (pkt) transport->deallocatePacket(pkt);
		Sleep(10);
		if (i == 499)
		{
			app.DebugPrintf("RakNet: Timeout waiting for smallId from host\n");
			transport->disconnect();
			return CGameNetworkManager::JOINGAME_FAIL_GENERAL;
		}
	}

	app.DebugPrintf("RakNet: Connected to %s:%d, assigned smallId=%d\n", hostIP, hostPort, localSmallId);
	...
	// Register the client's socket with the adapter for inbound data from host
	NetworkAdapter* adapter = g_NetworkManager.GetAdapter();
	if (adapter)
	{
		INetworkPlayer *localNp = getNetworkPlayer(&IQNet::m_player[localSmallId]);
		if (localNp && localNp->GetSocket())
		{
			adapter->registerConnection(m_rakNetHostConnection, localNp->GetSocket(), localNp);
		}
	}
...
wstring CPlatformNetworkManagerStub::GatherRTTStats()
{
	...
	for(unsigned int i = 0; i < (unsigned int)GetPlayerCount(); ++i)
	{
