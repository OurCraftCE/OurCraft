...
#include "..\..\World\Tiles\FurnaceTileEntity.h"
#include "..\..\World\Container\Container.h"
#include "..\..\World\Tiles\DispenserTileEntity.h"
#include "..\..\World\Tiles\SignTileEntity.h"
...
#include "..\..\World\IO\InputOutputStream.h"
...
#include "../Textures/ArchiveFile.h"
#include "..\OurCraft.h"
#include "UI\UI.h"
...
#include "..\Windows64\Win64ProfileData.h"

//CMinecraftApp app;
unsigned int CMinecraftApp::m_uiLastSignInData = 0;

const float CMinecraftApp::fSafeZoneX = 64.0f; // 5% of 1280 (legacy constant)
const float CMinecraftApp::fSafeZoneY = 36.0f; // 5% of 720 (legacy constant)

float CMinecraftApp::getSafeZoneX()
{
	Minecraft *mc = Minecraft::GetInstance();
	if (mc != NULL && mc->width_phys > 0)
		return mc->width_phys * 0.05f;
	return fSafeZoneX;
}

float CMinecraftApp::getSafeZoneY()
{
	Minecraft *mc = Minecraft::GetInstance();
	if (mc != NULL && mc->height_phys > 0)
		return mc->height_phys * 0.05f;
	return fSafeZoneY;
}
...
// 	m_bRead_TMS_DLCINFO_XML=false;
	...
	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		m_eOptionsStatusA[i]=CStorageManager::eOptions_Callback_Idle;
	}
...
bool CMinecraftApp::LoadCraftingTableMenu(int iPad, shared_ptr<LocalPlayer> player, int x, int y, int z)
{
	bool success = true;
	...
	{
		initData->bSplitscreen = true;
		success = ui.NavigateToScene(iPad, eUIScene_CraftingTableMenu, initData);
	}
	else
	{
		initData->bSplitscreen = false;
...
{
	// On PC, use Win64ProfileData for persistent local storage
	g_Win64ProfileData.Initialize(GAME_DEFINED_PROFILE_DATA_BYTES);

	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		// Use PC profile data instead of console ProfileManager
		void* profileData = g_Win64ProfileData.GetGameDefinedData(i);
		if (profileData != NULL)
			GameSettingsA[i] = (GAME_SETTINGS *)profileData;
		else
			GameSettingsA[i] = (GAME_SETTINGS *)ProfileManager.GetGameDefinedProfileData(i);

		// If both failed, allocate a zeroed default
		if (GameSettingsA[i] == NULL)
		{
			static GAME_SETTINGS s_defaultSettings[XUSER_MAX_COUNT] = {};
			GameSettingsA[i] = &s_defaultSettings[i];
		}

		// clear the flag to say the settings have changed
		GameSettingsA[i]->bSettingsChanged=false;

		// On PC, if no saved profile exists (all zeros), apply sensible defaults
		if (GameSettingsA[i]->ucGamma == 0 && GameSettingsA[i]->ucSensitivity == 0)
		{
			SetGameSettings(i, eGameSetting_MusicVolume, DEFAULT_VOLUME_LEVEL);
			SetGameSettings(i, eGameSetting_SoundFXVolume, DEFAULT_VOLUME_LEVEL);
			SetGameSettings(i, eGameSetting_Gamma, 100);
			SetGameSettings(i, eGameSetting_Difficulty, 1);
			SetGameSettings(i, eGameSetting_Sensitivity_InGame, 100);
			SetGameSettings(i, eGameSetting_Sensitivity_InMenu, 100);
			SetGameSettings(i, eGameSetting_ViewBob, 1);
			SetGameSettings(i, eGameSetting_GamertagsVisible, 1);
			SetGameSettings(i, eGameSetting_ControlScheme, 0);
			SetGameSettings(i, eGameSetting_ControlInvertLook, 0);
			SetGameSettings(i, eGameSetting_Hints, 1);
			SetGameSettings(i, eGameSetting_Autosave, 2);
			SetGameSettings(i, eGameSetting_Tooltips, 1);
			SetGameSettings(i, eGameSetting_InterfaceOpacity, 80);
			SetGameSettings(i, eGameSetting_Clouds, 1);
			SetGameSettings(i, eGameSetting_Online, 1);
			SetGameSettings(i, eGameSetting_InviteOnly, 0);
			SetGameSettings(i, eGameSetting_FriendsOfFriends, 1);
			SetGameSettings(i, eGameSetting_BedrockFog, 0);
			SetGameSettings(i, eGameSetting_DisplayHUD, 1);
			SetGameSettings(i, eGameSetting_DisplayHand, 1);
			SetGameSettings(i, eGameSetting_CustomSkinAnim, 1);
			SetGameSettings(i, eGameSetting_DeathMessages, 1);
			SetGameSettings(i, eGameSetting_UISize, 1);
			SetGameSettings(i, eGameSetting_UISizeSplitscreen, 2);
			SetGameSettings(i, eGameSetting_AnimatedCharacter, 1);
			GameSettingsA[i]->ucCurrentFavoriteSkinPos = 0;
			for (int j = 0; j < MAX_FAVORITE_SKINS; j++)
				GameSettingsA[i]->uiFavoriteSkinA[j] = 0xFFFFFFFF;
			GameSettingsA[i]->uiMashUpPackWorldsDisplay = 0xFFFFFFFF;
		}
	}
}
...
int CMinecraftApp::OptionsDataCallback(LPVOID pParam,int iPad,unsigned short usVersion,CStorageManager::eOptionsCallback eStatus)
{
	CMinecraftApp *pApp=(CMinecraftApp *)pParam;
...
CStorageManager::eOptionsCallback CMinecraftApp::GetOptionsCallbackStatus(int iPad)
{
	return m_eOptionsStatusA[iPad];
}

void CMinecraftApp::SetOptionsCallbackStatus(int iPad, CStorageManager::eOptionsCallback eStatus)
{
...
void CMinecraftApp::CheckGameSettingsChanged(bool bOverride5MinuteTimer, int iPad)
{
	...
	if(iPad==XUSER_INDEX_ANY)
	{
		for(int i=0;i<XUSER_MAX_COUNT;i++)
		{
			...
			{
				// Save to PC profile file
				g_Win64ProfileData.Save(i);
...
#endif // _DEBUG_MENUS_ENABLED

int CMinecraftApp::DisplaySavingMessage(void *pParam, CStorageManager::ESavingMessage eVal, int iPad)
{
...
void CMinecraftApp::HandleXuiActions(void)
{
	...
	// Process pending Discord join requests on the main thread
	if (DiscordManager::Get().HasPendingJoin())
	{
		char secret[128] = {0};
		DiscordManager::Get().GetPendingJoinSecret(secret, sizeof(secret));

		char ip[64] = {0};
		int port = 0;
		unsigned long long sessionId = 0;

		if (DiscordManager::Get().ParseJoinSecret(secret, ip, sizeof(ip), &port, &sessionId))
		{
			DebugPrintf("[Discord] Processing pending join: ip=%s port=%d\n", ip, port);
			// TODO: Implement actual join flow via g_NetworkManager when
			// IP-based connection support is fully wired up.
		}
		DiscordManager::Get().ClearPendingJoin();
	}
	...
	if(eAction!=eAppAction_Idle)
	{
		...
		case eAppAction_DisplayLavaMessage:
			// Display a warning about placing lava in the spawn area
			{
				...
				CStorageManager::EMessageResult result = ui.RequestMessageBox( IDS_CANT_PLACE_NEAR_SPAWN_TITLE, IDS_CANT_PLACE_NEAR_SPAWN_TEXT, uiIDA,1,XUSER_INDEX_ANY,NULL,NULL, app.GetStringTable());
				if(result != CStorageManager::EMessage_Busy) SetGlobalXuiAction(eAppAction_Idle);
...
// 					ui.RequestMessageBox(IDS_RETURNEDTOMENU_TITLE, IDS_RETURNEDTOTITLESCREEN_TEXT, uiIDA, 1, i,&CMinecraftApp::PrimaryPlayerSignedOutReturned,this,app.GetStringTable());
					...
					if(!g_NetworkManager.IsLeavingGame())
					{
						...
						if( g_NetworkManager.IsHost() )
						{
							...
							else if(!g_NetworkManager.IsLocalGame() && !g_NetworkManager.IsInGameplay() )
							{
								...
								for(int i = 0; i < (int)pMinecraft->levels.length; i++)
								{
			...
			case eAppAction_FailedToJoinNoPrivileges:
				{
					UINT uiIDA[1];
					uiIDA[0]=IDS_CONFIRM_OK;
					CStorageManager::EMessageResult result = ui.RequestMessageBox( IDS_NO_MULTIPLAYER_PRIVILEGE_TITLE, IDS_NO_MULTIPLAYER_PRIVILEGE_JOIN_TEXT, uiIDA,1,ProfileManager.GetPrimaryPad(),NULL,NULL, app.GetStringTable());
					if(result != CStorageManager::EMessage_Busy) SetAction(i,eAppAction_Idle);
				}
				break;
			...
			case eAppAction_LevelInBanLevelList:
				{			
					...
					if(player)
					{
						...
						CStorageManager::EMessageResult result = ui.RequestMessageBox( IDS_BANNED_LEVEL_TITLE, IDS_PLAYER_BANNED_LEVEL, uiIDA,2,i,&CMinecraftApp::BannedLevelDialogReturned,this, app.GetStringTable(),wchFormat);
						if(result != CStorageManager::EMessage_Busy) SetAction(i,eAppAction_Idle);
					}
					else
					{
		...
		{			
			...
			case eTMSAction_TMSPP_RetrieveFiles_RunPlayGame:
				...
				app.TMSPP_RetrieveFileList(i,CStorageManager::eGlobalStorage_Title,eTMSAction_TMSPP_UserFileList);
				break;

			case eTMSAction_TMSPP_UserFileList:
				// retrieve the file list first
				SetTMSAction(i,eTMSAction_TMSPP_UserFileList_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorageManager::eGlobalStorage_TitleUser,eTMSAction_TMSPP_DLCFile);
				break;
			...
			case eTMSAction_TMSPP_RetrieveFiles_DLCMain:
				// retrieve the file list first
				SetTMSAction(i,eTMSAction_TMSPP_GlobalFileList_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorageManager::eGlobalStorage_Title,eTMSAction_TMSPP_RetrieveUserFilelist_DLCFileOnly);
				break;
			case eTMSAction_TMSPP_RetrieveUserFilelist_DLCFileOnly:
				//StorageManager.TMSPP_DeleteFile(i,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,L"TP06.png",NULL,NULL, 0);
				SetTMSAction(i,eTMSAction_TMSPP_UserFileList_Waiting);
				app.TMSPP_RetrieveFileList(i,CStorageManager::eGlobalStorage_TitleUser,eTMSAction_TMSPP_DLCFileOnly);
...
int CMinecraftApp::BannedLevelDialogReturned(void *pParam,int iPad,const CStorageManager::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	//Minecraft *pMinecraft=Minecraft::GetInstance();
...
void CMinecraftApp::loadMediaArchive()
{
	...
	if (!mediapath.empty())
	{
		DebugPrintf("loadMediaArchive: Opening %ls\n", mediapath.c_str());
		m_mediaArchive = new ArchiveFile( File(mediapath) );
		DebugPrintf("loadMediaArchive: Archive loaded OK, ptr=%p\n", m_mediaArchive);
	}
#if 0
...
int CMinecraftApp::PrimaryPlayerSignedOutReturned(void *pParam,int iPad,const CStorageManager::EMessageResult)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	//Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::EthernetDisconnectReturned(void *pParam,int iPad,const CStorageManager::EMessageResult)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::UnlockFullInviteReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
	bool bNoPlayer;
...
int CMinecraftApp::UnlockFullSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	//CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::UnlockFullExitReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::TrialOverReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::MarketplaceCountsCallback(LPVOID pParam,CStorageManager::DLC_TMS_DETAILS *pTMSDetails, int iPad)
{
...
// #ifdef _CONTENT_PACKAGE
// 			 CStorageManager::ETMSStatus eTMSStatus;
// 			 eTMSStatus=StorageManager.ReadTMSFile(ProfileManager.GetPrimaryPad(),CStorageManager::eGlobalStorage_Title,CStorageManager::eTMS_FileType_Graphic, L"Default_Cape.png",&pBuffer, &dwSize);
// 			 if(eTMSStatus==CStorageManager::ETMSStatus_Idle)
// 			 {
...
//  }
 ...
 void CMinecraftApp::HandleDLC(DLCPack *pack)
 {
	...
	swprintf(szFullFilename, MAX_PATH, L"DLC\\%ls\\*", pack->getName().c_str());
	HANDLE hFind = FindFirstFileW(szFullFilename, &wfd);
	if(hFind != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			{
				swprintf(szFullFilename, MAX_PATH, L"DLC\\%ls\\%ls", pack->getName().c_str(), wfd.cFileName);
...
//  int CMinecraftApp::DLCReadCallback(LPVOID pParam,CStorageManager::DLC_FILE_DETAILS *pDLCData)
//  {
//
//
// 	 return 0;
...
void CMinecraftApp::InitLocalDLC()
{
	m_dlcInstaller.SetPaths(L"dlc\\available", L"dlc\\installed");

	// Ensure the dlc directories exist
	CreateDirectoryW(L"dlc", NULL);
	CreateDirectoryW(L"dlc\\available", NULL);
	CreateDirectoryW(L"dlc\\installed", NULL);

	// Load the catalog
	m_dlcCatalog.Refresh(L"dlc\\catalog.ini", L"dlc\\installed");
	app.DebugPrintf("InitLocalDLC: catalog has %d entries\n", m_dlcCatalog.GetEntryCount());

	// Auto-load all installed DLC packs from dlc/installed/
	int loadedCount = m_dlcInstaller.LoadAllInstalled(m_dlcManager);
	app.DebugPrintf("InitLocalDLC: auto-loaded %d installed DLC packs\n", loadedCount);

	// Register texture packs from loaded DLC
	if (loadedCount > 0)
	{
		Minecraft *pMinecraft = Minecraft::GetInstance();
		if (pMinecraft && pMinecraft->skins)
		{
			DWORD packCount = m_dlcManager.getPackCount(DLCManager::e_DLCType_All);
			for (DWORD i = 0; i < packCount; i++)
			{
				DLCPack *pack = m_dlcManager.getPack(i, DLCManager::e_DLCType_All);
				if (pack && pack->getDLCItemsCount(DLCManager::e_DLCType_Texture) > 0)
				{
					pMinecraft->skins->addTexturePackFromDLC(pack, pack->GetPackId());
					app.DebugPrintf("InitLocalDLC: registered texture pack from %ls\n", pack->getName().c_str());
				}
			}
		}
	}
...
bool CMinecraftApp::InstallLocalDLCPack(const wstring &packId)
{
	DLCInstaller::EInstallStatus status = m_dlcInstaller.InstallFromLocal(packId);
	if (status != DLCInstaller::eStatus_Installed)
		return false;

	// Load the pack into DLCManager
	DLCPack *pack = m_dlcInstaller.LoadInstalledPack(packId, m_dlcManager);
	if (pack == NULL)
		return false;

	// Update catalog
	m_dlcCatalog.MarkInstalled(packId, true);

	// If it has texture pack data, add to texture pack repository
	if (pack->getDLCItemsCount(DLCManager::e_DLCType_Texture) > 0)
	{
		Minecraft *pMinecraft = Minecraft::GetInstance();
		if (pMinecraft && pMinecraft->skins)
...
bool CMinecraftApp::UninstallLocalDLCPack(const wstring &packId)
{
	// Remove from DLCManager
	DLCPack *pack = m_dlcManager.getPack(packId);
	if (pack != NULL)
	{
		m_dlcManager.removePack(pack);
	}

	// Remove files
	bool result = m_dlcInstaller.Uninstall(packId);

	// Update catalog
	m_dlcCatalog.MarkInstalled(packId, false);
...
// bool CMinecraftApp::UploadFileToGlobalStorage(int iQuadrant, CStorageManager::eGlobalStorage eStorageFacility, wstring *wsFile  )
// {
// 	bool bRes=false;
// #ifndef _CONTENT_PACKAGE
// 	// read the local file
...
int CMinecraftApp::ExitAndJoinFromInvite(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	CMinecraftApp* pApp = (CMinecraftApp*)pParam;
	//Minecraft *pMinecraft=Minecraft::GetInstance();
...
int CMinecraftApp::ExitAndJoinFromInviteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	CMinecraftApp *pClass = (CMinecraftApp *)pParam;
...
int CMinecraftApp::WarningTrialTexturePackReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	return 0;
}

int CMinecraftApp::ExitAndJoinFromInviteAndSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	//CMinecraftApp* pClass = (CMinecraftApp*)pParam;
...
int CMinecraftApp::ExitAndJoinFromInviteDeclineSaveReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	// results switched for this dialog
	if(result==CStorageManager::EMessage_ResultDecline) 
	{
...
unordered_map<PlayerUID,MOJANG_DATA *, PlayerUID_Hash > CMinecraftApp::MojangData;
...
HRESULT CMinecraftApp::RegisterMojangData(WCHAR *pXuidName, PlayerUID xuid, WCHAR *pSkin, WCHAR *pCape)
{
	...
	if(eTempXuid!=eXUID_Undefined)
	{
		...
		wcsncpy( pMojangData->wchSkin, pSkin, MAX_CAPENAME_SIZE - 1);
		pMojangData->wchSkin[MAX_CAPENAME_SIZE - 1] = L'\0';
		wcsncpy( pMojangData->wchCape, pCape, MAX_CAPENAME_SIZE - 1);
		pMojangData->wchCape[MAX_CAPENAME_SIZE - 1] = L'\0';
...
int CMinecraftApp::ExitGameFromRemoteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
...
{
	PlayerUID xuid= _wcstoui64(pBannedListData->wchPlayerUID, NULL, 10);
...
void CMinecraftApp::AddLevelToBannedLevelList(int iPad, PlayerUID xuid, char *pszLevelName, bool bWriteToTMS)
{
	...
	swprintf(pBannedListData->wchPlayerUID, 64, L"%llu", (unsigned long long)xuid);
	strncpy(pBannedListData->pszLevelName, pszLevelName, sizeof(pBannedListData->pszLevelName) - 1);
	pBannedListData->pszLevelName[sizeof(pBannedListData->pszLevelName) - 1] = '\0';
	...
	{	
		...
		//bool bRes=StorageManager.WriteTMSFile(iPad,CStorageManager::eGlobalStorage_TitleUser,L"BannedList",(PBYTE)pBannedList, dwDataBytes);
		StorageManager.TMSPP_WriteFile(iPad,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,L"BannedList",(PBYTE) pBannedList, dwDataBytes,NULL,NULL, 0);
	}
...
bool CMinecraftApp::IsInBannedLevelList(int iPad, PlayerUID xuid, char *pszLevelName)
{
	...
	{
		PBANNEDLISTDATA pData=*it;
		PlayerUID bannedPlayerUID = _wcstoui64(pData->wchPlayerUID, NULL, 10);
...
void CMinecraftApp::RemoveLevelFromBannedLevelList(int iPad, PlayerUID xuid, char *pszLevelName)
{
	...
	for(AUTO_VAR(it, m_vBannedListA[iPad]->begin()); it != m_vBannedListA[iPad]->end(); )
	{
		...
		{
			PlayerUID bannedPlayerUID = _wcstoui64(pBannedListData->wchPlayerUID, NULL, 10);
	...
	{
		// wipe the file
		StorageManager.TMSPP_DeleteFile(iPad,CStorageManager::eGlobalStorage_TitleUser,CStorageManager::TMS_FILETYPE_BINARY,L"BannedList",NULL,NULL, 0);
	}
	else
	{
				...
				{			
					...
					{	
						...
						{			
							...
							//pTMSPPreq->eStorageFacility=CStorageManager::eGlobalStorage_TitleUser;
							pTMSPPreq->eStorageFacility=CStorageManager::eGlobalStorage_Title;
							pTMSPPreq->eFileTypeVal=CStorageManager::TMS_FILETYPE_BINARY;
...
bool CMinecraftApp::RetrieveNextDLCContent()
{
	...
	for(AUTO_VAR(it, m_DLCDownloadQueue.begin()); it != m_DLCDownloadQueue.end(); ++it)
	{
		...
		if(pCurrent->eState==e_DLC_ContentState_Idle)
		{
			...
			CStorageManager::EDLCStatus status = StorageManager.GetDLCOffers(ProfileManager.GetPrimaryPad(), &CMinecraftApp::DLCOffersReturned, this, pCurrent->dwType);
			if(status==CStorageManager::EDLC_Pending)
			{
...
int CMinecraftApp::TMSPPFileReturned(LPVOID pParam,int iPad,int iUserData,LPVOID lpFileData, WCHAR *wchFilename)
{
	CStorageManager::PTMSPP_FILEDATA pFileData = (CStorageManager::PTMSPP_FILEDATA)lpFileData;
	CMinecraftApp* pClass = (CMinecraftApp *) pParam;
...
bool CMinecraftApp::RetrieveNextTMSPPContent()
{
	...
	for(AUTO_VAR(it, m_TMSPPDownloadQueue.begin()); it != m_TMSPPDownloadQueue.end(); ++it)
	{
		...
		{
			CStorageManager::ETMSStatus status;
			app.DebugPrintf("RetrieveNextTMSPPContent - type = %d, %ls\n",pCurrent->eType,pCurrent->wchFilename);
			//CStorageManager::ETMSStatus status=StorageManager.TMSPP_ReadFile(ProfileManager.GetPrimaryPad(),pCurrent->eStorageFacility,pCurrent->eFileTypeVal,pCurrent->wchFilename,pCurrent->CallbackFunc,this,0);
			if(0)//wcscmp(pCurrent->wchFilename,L"TP01.png")==0)
			{
				...
				status=CStorageManager::ETMSStatus_Fail;
			}
			else
			{
			...
			switch(status)
			{
			case CStorageManager::ETMSStatus_Pending:
				pCurrent->eState=e_TMS_ContentState_Retrieving;
				break;
			case CStorageManager::ETMSStatus_Idle:
				pCurrent->eState=e_TMS_ContentState_Retrieved;
				break;
			case CStorageManager::ETMSStatus_Fail_ReadInProgress:
				// try again next time
				{
...
int CMinecraftApp::TexturePackDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	return 0;
}
...
void CMinecraftApp::getLocale(vector<wstring> &vecWstrLocales)
{
	...
	unsigned char ucUserLang = GetMinecraftLanguage(ProfileManager.GetPrimaryPad());
	if (ucUserLang != 0)
	{
		// User has a language preference - use it as the primary locale
		eMCLang userLang = (eMCLang)ucUserLang;
		locales.push_back(userLang);
