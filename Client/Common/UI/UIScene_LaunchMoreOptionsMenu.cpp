...
UIScene_LaunchMoreOptionsMenu::UIScene_LaunchMoreOptionsMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	if(m_loadedResolution == eSceneResolution_1080)
	{
		...
		{
			wchar_t viewDistLabel[32];
			swprintf(viewDistLabel, 32, L"%d", m_params->worldSize);
			m_sliderWorldSize.init(viewDistLabel, eControl_WorldSize, PlayerChunkMap::MIN_VIEW_DISTANCE, PlayerChunkMap::MAX_VIEW_DISTANCE, m_params->worldSize);
		}
...
void UIScene_LaunchMoreOptionsMenu::handleSliderMove(F64 sliderId, F64 currentValue)
{
	...
	case eControl_WorldSize:
		...
		{
			wchar_t viewDistLabel[32];
			swprintf(viewDistLabel, 32, L"%d", value);
			m_sliderWorldSize.setLabel(viewDistLabel);
		}
		break;
	}
}
