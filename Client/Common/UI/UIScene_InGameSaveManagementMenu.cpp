	...
	if(hasFocus(m_iPad))
	{
		...
		if(!m_bSavesDisplayed)
		{
			...
			if(m_pSaveDetails!=NULL)
			{
				...
				for(int i = 0; i < m_pSaveDetails->iSaveC; ++i)
				{
					m_spaceIndicatorSaves.addSave( m_pSaveDetails->SaveInfoA[i].metaData.dataSize );
...
int UIScene_InGameSaveManagementMenu::DeleteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
