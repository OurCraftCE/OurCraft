...
typedef struct _SaveListDetails
{
	...
	_SaveListDetails()
	{
		...
		ZeroMemory(UTF8SaveName,sizeof(char)*128);
		ZeroMemory(UTF8SaveFilename,sizeof(char)*MAX_SAVEFILENAME_LENGTH);
	}
...
typedef struct _MessageBoxInfo
{
	...
	UINT uiOptionC;
	DWORD dwPad;
	int( *Func)(LPVOID,int,const CStorageManager::EMessageResult);
	LPVOID lpParam;
