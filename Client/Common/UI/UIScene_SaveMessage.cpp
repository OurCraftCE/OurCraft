	...
	case PROFILE_LOADED_TIMER_ID:
		{
			...
			{
			case CStorageManager::eOptions_Callback_Idle:
			case CStorageManager::eOptions_Callback_Success:
				// set defaults - which has already been done
				killTimer(PROFILE_LOADED_TIMER_ID);
				ui.NavigateToHomeMenu();
				app.SetOptionsCallbackStatus(0,CStorageManager::eOptions_Callback_Idle);
				break;
			case CStorageManager::eOptions_Callback_Read_CorruptDeleted:
				killTimer(PROFILE_LOADED_TIMER_ID);
				ui.NavigateToHomeMenu();
				app.SetOptionsCallbackStatus(0,CStorageManager::eOptions_Callback_Idle);
				break;
			case CStorageManager::eOptions_Callback_Read_Corrupt:
				// get the user to delete the options file
				app.DebugPrintf("Corrupt options file\n");
				app.SetOptionsCallbackStatus(0,CStorageManager::eOptions_Callback_Read_CorruptDeletePending);
				m_bIgnoreInput=false;
...
int UIScene_SaveMessage::DeleteOptionsDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	app.SetOptionsCallbackStatus(0,CStorageManager::eOptions_Callback_Read_CorruptDeleted);
	return 0;
}
