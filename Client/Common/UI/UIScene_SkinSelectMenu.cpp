...
int UIScene_SkinSelectMenu::getNextSkinIndex(DWORD sourceIndex)
{
	...
	case SKIN_SELECT_PACK_FAVORITES:
		++nextSkin;
		if(nextSkin>=(int)app.GetPlayerFavoriteSkinsCount(m_iPad))
		{
			nextSkin=0;
		}
	...
	default:
		...
		else if(m_currentPack != NULL && nextSkin>=(int)m_currentPack->getSkinCount())
		{
			nextSkin = 0;
		}		
		break;
...
int UIScene_SkinSelectMenu::getNextPackIndex(DWORD sourceIndex)
{
	...
	++nextPack;
	if(nextPack > (int)app.m_dlcManager.getPackCount(DLCManager::e_DLCType_Skin) - 1 + SKIN_SELECT_MAX_DEFAULTS)
	{
...
int UIScene_SkinSelectMenu::UnlockSkinReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
...
void UIScene_SkinSelectMenu::AddFavoriteSkin(int iPad,int iSkinID)
{
	...
	for(int i=0;i<(int)uiCurrentFavoriteSkinsCount;i++)
	{
