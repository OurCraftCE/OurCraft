...
#include "..\..\OurCraft.h"
#include "..\..\..\World\FwdHeaders/net.minecraft.world.entity.boss.enderdragon.h"
#include "../../Input/KeyboardMouseInput.h"
#include "UIControl_Slider.h"
#include "../../Mobs/EnderDragonRenderer.h"
...
UIController::UIController()
{
	...
	// Default to 1920x1080 for Windows PC; will be updated by preInit or lazy init from Minecraft instance
...
void UIController::tick()
{
	...
	for(unsigned int i = 0; i < eUIGroup_COUNT; ++i)
	{
		if(m_bCloseAllScenes[i])
		{
			...
			if(m_groups[i]->getTooltips()) m_groups[i]->getTooltips()->SetTooltips(-1);
			m_bCloseAllScenes[i] = false;
		}
	}
...
void UIController::loadSkins()
{
	...
	app.DebugPrintf("loadSkins: m_fScreenHeight=%f\n", m_fScreenHeight);
	...
	app.DebugPrintf("loadSkins: platformSkinPath=%ls\n", platformSkinPath.c_str());
	...
	// Always load both platform skins - the HD fallback skins below need platformskinHD.swf
	// even when screen height isn't exactly 1080 (e.g. windowed mode with title bar)
	if(m_fScreenHeight!=1080.0f)
	{
		loadSkin(L"skinHDWin.swf", L"platformskinHD.swf");
	}
	else
	{
		loadSkin(L"skinWin.swf", L"platformskin.swf");
	}
...
IggyLibrary UIController::loadSkin(const wstring &skinPath, const wstring &skinName)
{
	...
	app.DebugPrintf("loadSkin: Checking %ls (empty=%d, inArchive=%d)\n", skinPath.c_str(), skinPath.empty() ? 1 : 0, app.hasArchiveFile(skinPath) ? 1 : 0);
	if(!skinPath.empty() && app.hasArchiveFile(skinPath))
	{
		byteArray baFile = app.getArchiveFile(skinPath);
		app.DebugPrintf("loadSkin: Loading %ls as %ls (%d bytes)\n", skinPath.c_str(), skinName.c_str(), baFile.length);
		lib = IggyLibraryCreateFromMemoryUTF16( (IggyUTF16 *)skinName.c_str() , (void *)baFile.data, baFile.length, NULL );
		app.DebugPrintf("loadSkin: Result lib=%d\n", (int)lib);
...
#endif
		{
			if (!g_KBMInput.IsMouseGrabbed())
			{
				...
				if (pScene && pScene->getMovie())
				{
					...
					// Mouse right-click triggers menu back/cancel
					if (g_KBMInput.IsMouseButtonPressed(2))
					{
						handleKeyPress(0, ACTION_MENU_CANCEL);
					}
				...
				// Forward character input to Iggy for text fields
				if (pScene && pScene->getMovie())
				{
					Iggy *movie = pScene->getMovie();
					while (g_KBMInput.HasCharInput())
					{
						wchar_t ch = g_KBMInput.PopCharInput();
						IggyEvent charEvent;
						IggyMakeEventChar(&charEvent, (S32)ch);
						IggyEventResult result;
						IggyPlayerDispatchEventRS(movie, &charEvent, &result);
					}
				}
			}
			handleInput();
...
void UIController::handleInput()
{
	...
	// Unmapped controller loop disabled — Durango-only feature, causes phantom input on Windows
}
...
void UIController::handleKeyPress(unsigned int iPad, unsigned int key)
{
	...
	if (iPad == 0)
	{
		...
		if ((key == ACTION_MENU_OK || key == ACTION_MENU_A) && (!g_KBMInput.IsMouseGrabbed() || g_KBMInput.IsCursorHiddenForUI()))
		{
...
void UIController::setupCustomDrawMatrices(UIScene *scene, CustomDrawData *customDrawRegion)
{
	...
	{
		left = (LONG)(m_tileOriginX + (sceneWidth + customDrawRegion->mat[(0*4)+3]*sceneWidth)/2);
		right = (LONG)(left + ( (sceneWidth * customDrawRegion->mat[0])/2 ) * customDrawRegion->x1);
	}

	top = (LONG)(m_tileOriginY + (sceneHeight - customDrawRegion->mat[(1*4)+3]*sceneHeight)/2);
	bottom = (LONG)(top + (sceneHeight * -customDrawRegion->mat[(1*4) + 1])/2 * customDrawRegion->y1);
...
GDrawTexture * RADLINK UIController::TextureSubstitutionCreateCallback ( void * user_callback_data , IggyUTF16 * texture_name , S32 * width , S32 * height , void * * destroy_callback_data )
{
	...
	if(it != uiController->m_substitutionTextures.end())
	{
		...
		if( image.getData() != NULL )
		{
			...
			*destroy_callback_data = (void *)(uintptr_t)id;
...
bool UIController::NavigateToScene(int iPad, EUIScene scene, void *initData, EUILayer layer, EUIGroup group)
{
	...
	if(GetMenuDisplayed(iPad))
	{
		...
		case eUIScene_CraftingTableMenu:
...
void UIController::CloseAllPlayersScenes()
{
	...
	if(m_groups[(int)eUIGroup_Fullscreen] && m_groups[(int)eUIGroup_Fullscreen]->getTooltips()) m_groups[(int)eUIGroup_Fullscreen]->getTooltips()->SetTooltips(-1);
	for(unsigned int i = 0; i < eUIGroup_COUNT; ++i)
	{
		//m_bCloseAllScenes[i] = true;
		m_groups[i]->closeAllScenes();
...
void UIController::CloseUIScenes(int iPad, bool forceIPad)
{
	...
	if(!m_groups[(int)group]) return;

	m_groups[(int)group]->closeAllScenes();
	if(m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->SetTooltips(-1);
...
void UIController::SetTooltipText( unsigned int iPad, unsigned int tooltip, int iTextID )
{
	...
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->SetTooltipText(tooltip, iTextID);
}
...
void UIController::SetEnableTooltips( unsigned int iPad, BOOL bVal )
{
	...
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->SetEnableTooltips(bVal);
}
...
void UIController::ShowTooltip( unsigned int iPad, unsigned int tooltip, bool show )
{
	...
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->ShowTooltip(tooltip,show);
}
...
void UIController::SetTooltips( unsigned int iPad, int iA, int iB, int iX, int iY, int iLT, int iRT, int iLB, int iRB, int iLS, bool forceUpdate)
{
	...
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->SetTooltips(iA, iB, iX, iY, iLT, iRT, iLB, iRB, iLS, forceUpdate);
}
...
void UIController::EnableTooltip( unsigned int iPad, unsigned int tooltip, bool enable )
{
	...
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->EnableTooltip(tooltip,enable);
}
...
void UIController::AnimateKeyPress(int iPad, int iAction, bool bRepeat, bool bPressed, bool bReleased)
{
	...
	bool handled = false;
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->handleInput(iPad, iAction, bRepeat, bPressed, bReleased, handled);
}
...
void UIController::OverrideSFX(int iPad, int iAction,bool bVal)
{
	...
	bool handled = false;
	if(m_groups[(int)group] && m_groups[(int)group]->getTooltips()) m_groups[(int)group]->getTooltips()->overrideSFX(iPad, iAction,bVal);
}
...
void UIController::ShowSavingMessage(unsigned int iPad, CStorageManager::ESavingMessage eVal)
{
	if(!m_uiReady) return;
	bool show = false;
	switch(eVal)
	{
	case CStorageManager::ESavingMessage_None:
		show = false;
		break;
	case CStorageManager::ESavingMessage_Short:
	case CStorageManager::ESavingMessage_Long:
		show = true;
		break;
	}
...
CStorageManager::EMessageResult UIController::RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad,
														   int( *Func)(LPVOID,int,const CStorageManager::EMessageResult),LPVOID lpParam, C4JStringTable *pStringTable, WCHAR *pwchFormatString,DWORD dwFocusButton, bool bIsError)
{
	...
	if( completed )
	{
		...
		return CStorageManager::EMessage_Pending;
	}
	else
	{
		return CStorageManager::EMessage_Busy;
	}
}

CStorageManager::EMessageResult UIController::RequestUGCMessageBox(UINT title/* = -1 */, UINT message/* = -1 */, int iPad/* = -1*/, int( *Func)(LPVOID,int,const CStorageManager::EMessageResult)/* = NULL*/, LPVOID lpParam/* = NULL*/)
{
...
CStorageManager::EMessageResult UIController::RequestContentRestrictedMessageBox(UINT title/* = -1 */, UINT message/* = -1 */, int iPad/* = -1*/, int( *Func)(LPVOID,int,const CStorageManager::EMessageResult)/* = NULL*/, LPVOID lpParam/* = NULL*/)
{
	// Default title / messages
	if (title == -1)
	{
...
const wchar_t* UIController::GetPromptText(const InputPrompt& prompt)
{
	return InputPrompts::GetText(prompt, g_KBMInput.IsKBMActive());
}
