...
public:
	...
	virtual void ShowSavingMessage(unsigned int iPad, CStorageManager::ESavingMessage eVal);
	...
	virtual CStorageManager::EMessageResult RequestMessageBox(UINT uiTitle, UINT uiText, UINT *uiOptionA,UINT uiOptionC, DWORD dwPad=XUSER_INDEX_ANY,
						int( *Func)(LPVOID,int,const CStorageManager::EMessageResult)=NULL,LPVOID lpParam=NULL, C4JStringTable *pStringTable=NULL, WCHAR *pwchFormatString=NULL,DWORD dwFocusButton=0, bool bIsError = true);

	CStorageManager::EMessageResult RequestUGCMessageBox(UINT title = -1, UINT message = -1, int iPad = -1, int( *Func)(LPVOID,int,const CStorageManager::EMessageResult) = NULL, LPVOID lpParam = NULL);
	CStorageManager::EMessageResult RequestContentRestrictedMessageBox(UINT title = -1, UINT message = -1, int iPad = -1, int( *Func)(LPVOID,int,const CStorageManager::EMessageResult) = NULL, LPVOID lpParam = NULL);
...
public:
	...
	// Returns the appropriate prompt text for the current input mode (KBM vs gamepad)
	const wchar_t* GetPromptText(const InputPrompt& prompt);
