...
void UIScene_MessageBox::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_CANCEL:
		...
		{
			navigateBack();
			if(m_Func) m_Func(m_lpParam, iPad, CStorageManager::EMessage_Cancelled);
		}
		break;
	case ACTION_MENU_OK:
...
{
	CStorageManager::EMessageResult result = CStorageManager::EMessage_Cancelled;
	switch((int)controlId)
	{
	case 0:
		result = CStorageManager::EMessage_ResultAccept;
		break;
	case 1:
		result = CStorageManager::EMessage_ResultDecline;
		break;
	case 2:
		result = CStorageManager::EMessage_ResultThirdOption;
		break;
	case 3:
		result = CStorageManager::EMessage_ResultFourthOption;
		break;
	}
