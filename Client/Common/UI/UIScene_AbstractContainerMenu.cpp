...
void UIScene_AbstractContainerMenu::tick()
{
	...
	// Dispatch mouse button events to Iggy so Flash UI elements (tabs, buttons) respond to clicks
	if (g_KBMInput.IsMouseButtonPressed(KeyboardMouseInput::MOUSE_LEFT))
	{
		IggyEvent clickEvent;
		IggyMakeEventMouseButton(&clickEvent, IGGY_MOUSEBUTTON_LeftDown);
		IggyPlayerDispatchEventRS(getMovie(), &clickEvent, &result);
	}
	if (g_KBMInput.IsMouseButtonReleased(KeyboardMouseInput::MOUSE_LEFT))
	{
		IggyEvent clickEvent;
		IggyMakeEventMouseButton(&clickEvent, IGGY_MOUSEBUTTON_LeftUp);
		IggyPlayerDispatchEventRS(getMovie(), &clickEvent, &result);
	}
}
