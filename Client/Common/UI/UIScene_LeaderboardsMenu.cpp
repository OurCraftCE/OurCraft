...
void UIScene_LeaderboardsMenu::updateTooltips()
{
	...
	{
		int selection = m_newSel;
		if( selection >= GetEntryStartIndex() &&
			selection < (int)(GetEntryStartIndex() + m_leaderboard.m_entries.size()) )
		{
...
void UIScene_LeaderboardsMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_DOWN:
		++m_newSel;
		if(m_newSel>=(int)m_leaderboard.m_totalEntryCount) m_newSel = m_leaderboard.m_totalEntryCount - 1;
		sendInputToMovie(key, repeat, pressed, released);
		break;
...
bool UIScene_LeaderboardsMenu::RetrieveStats()
{
	...
	{
		if(m_newEntryIndex < (unsigned int)GetEntryStartIndex() && m_newEntryIndex == 1)
		{
		...
		if(m_newEntryIndex < (unsigned int)GetEntryStartIndex())
		{
			insertPosition = 0;
		...
		{
			insertPosition = (unsigned int)m_leaderboard.m_entries.size();
...
void UIScene_LeaderboardsMenu::handleRequestMoreData(F64 startIndex, bool up)
{
	...
	if( m_leaderboard.m_totalEntryCount > 0 && (item+1) < (unsigned int)GetEntryStartIndex() )
	{
	...
	else if( m_leaderboard.m_totalEntryCount > 0 && (item+1) >= (GetEntryStartIndex() + m_leaderboard.m_entries.size()) )
	{
		if( LeaderboardManager::Instance()->isIdle() )
		{
			int readIndex = (GetEntryStartIndex() + 1) + (int)m_leaderboard.m_entries.size();
			assert( readIndex >= 1 && readIndex <= (int)m_leaderboard.m_totalEntryCount );
			ReadStats(readIndex);
		}
	}
...
int UIScene_LeaderboardsMenu::ExitLeaderboards(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
