...
UIScene_LoadMenu::UIScene_LoadMenu(int iPad, void *initData, UILayer *parentLayer) : IUIScene_StartGame(iPad, parentLayer)
{
	...
	else
	{
		...
		else
		{
			...
			CStorageManager::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(&pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex],&LoadSaveDataThumbnailReturned,this,true);
			m_bShowTimer = true;
		}
...
void UIScene_LoadMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_OTHER_STICK_DOWN:
		...
		if(getSceneResolution() == eSceneResolution_1080)
		{
			...
			if (m_MoreOptionsParams.bOnlineGame != (BOOL)bOnlineGame)
			{
			...
			else // it's creative mode
			{
				...
				if(m_bHasBeenInCreative)
				{
					...
					else
					{
						...
						CStorageManager::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[(int)m_iSaveGameInfoIndex],&LoadSaveDataReturned,this);
...
int UIScene_LoadMenu::CheckResetNetherReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	...
	else if(result==CStorageManager::EMessage_ResultAccept)
	{
...
int UIScene_LoadMenu::ConfirmLoadReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
	...
	if(result==CStorageManager::EMessage_ResultAccept) 
	{
		...
		else
		{
			...
			CStorageManager::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[(int)pClass->m_iSaveGameInfoIndex],&LoadSaveDataReturned,pClass);
...
int UIScene_LoadMenu::TrophyDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
	return LoadDataComplete(pClass);
}

int UIScene_LoadMenu::DeleteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadMenu* pClass = (UIScene_LoadMenu*)pParam;
