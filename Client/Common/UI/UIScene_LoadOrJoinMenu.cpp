...
#include "..\..\..\World\FwdHeaders/net.minecraft.world.level.chunk.storage.h"
#include "..\..\..\World\IO\ConsoleSaveFile.h"
#include "..\..\..\World\IO\ConsoleSaveFileOriginal.h"
#include "..\..\..\World\IO\ConsoleSaveFileSplit.h"
...
CStorageManager::SAVETRANSFER_FILE_DETAILS UIScene_LoadOrJoinMenu::m_debugTransferDetails;
...
void UIScene_LoadOrJoinMenu::tick()
{
    ...
    if(!m_bSavesDisplayed && m_pSaveDetails != NULL && m_pSaveDetails->iSaveC > 0)
    {
        for(int i = 0; i < m_pSaveDetails->iSaveC; i++)
        {
            ...
            // Convert UTF8 to UTF16 for UI display
            ::MultiByteToWideChar(CP_UTF8, 0, m_pSaveDetails->SaveInfoA[i].UTF8SaveTitle, -1, m_saveDetails[i].UTF16SaveName, 128);
            ::MultiByteToWideChar(CP_UTF8, 0, m_pSaveDetails->SaveInfoA[i].UTF8SaveFilename, -1, m_saveDetails[i].UTF16SaveFilename, MAX_SAVEFILENAME_LENGTH);
        }
        m_controlSavesTimer.setVisible( false );
        ...
        if(!m_bExitScene && m_bSavesDisplayed && !m_bRetrievingSaveThumbnails && !m_bAllLoaded)
        {
            if( m_iRequestingThumbnailId < (m_buttonListSaves.getItemCount() - m_iDefaultButtonsC ))
            {
                ...
                CStorageManager::ESaveGameState eLoadStatus=StorageManager.LoadSaveDataThumbnail(&pSaveDetails->SaveInfoA[(int)m_iRequestingThumbnailId],&LoadSaveDataThumbnailReturned,this);

                if(eLoadStatus!=CStorageManager::ESaveGame_GetSaveThumbnail)
                {
...
void UIScene_LoadOrJoinMenu::GetSaveInfo()
{
    ...
    else
    {
        ...
        {
            CStorageManager::ESaveGameState eSGIStatus= StorageManager.GetSavesInfo(m_iPad,NULL,this,"save"); 
        }

#if TO_BE_IMPLEMENTED
        if(eSGIStatus==CStorageManager::ESGIStatus_NoSaves)
        {
            uiSaveC=0;
            m_controlSavesTimer.setVisible( false );
...
void UIScene_LoadOrJoinMenu::CheckAndJoinGame(int gameIndex)
{
	...
	{
		// Content restriction checks (stub - always allow on Windows)
		bool noUGC = false;
...
int UIScene_LoadOrJoinMenu::DeleteSaveDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
    ...
    if(result==CStorageManager::EMessage_ResultDecline && validSelection) 
    {
        if(app.DebugSettingsOn() && app.GetLoadSavesFromFolderEnabled())
        {
            pClass->m_bIgnoreInput=false;
...
int UIScene_LoadOrJoinMenu::SaveOptionsDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;
    ...
    switch(result)
    {
    case CStorageManager::EMessage_ResultDecline:  // rename
        {
    ...
    case CStorageManager::EMessage_ResultThirdOption:  // delete -
        {
            // delete the save game
            // Have to ask the player if they are sure they want to delete this game
            UINT uiIDA[2];
...
#ifdef SONY_REMOTE_STORAGE_UPLOAD
    case CStorageManager::EMessage_ResultFourthOption: // upload to cloud
        {
			UINT uiIDA[2];
			uiIDA[0]=IDS_CONFIRM_OK;
			uiIDA[1]=IDS_CONFIRM_CANCEL;
        ...
        break;
#else
    case CStorageManager::EMessage_ResultFourthOption: // copy save
        {
			UINT uiIDA[2];
			uiIDA[0]=IDS_CONFIRM_OK;
			uiIDA[1]=IDS_CONFIRM_CANCEL;
    ...
    case CStorageManager::EMessage_Cancelled:
    default:
        {
            // reset the tooltips
            pClass->updateTooltips();
...
int UIScene_LoadOrJoinMenu::TexturePackDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
    UIScene_LoadOrJoinMenu *pClass = (UIScene_LoadOrJoinMenu *)pParam;
    ...
    if(result==CStorageManager::EMessage_ResultAccept) 
    {
        ...
        if( result==CStorageManager::EMessage_ResultAccept ) // Full version
        {
...
int UIScene_LoadOrJoinMenu::CrossSaveFinishedCallback(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) pParam;
...
int UIScene_LoadOrJoinMenu::RemoteSaveNotFoundCallback(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) pParam;
	pClass->m_eSaveTransferState = eSaveTransfer_Idle;
	return 0;
...
int UIScene_LoadOrJoinMenu::DownloadSonyCrossSaveThreadProc( LPVOID lpParameter )
{
    ...
    while(pClass->m_eSaveTransferState!=eSaveTransfer_Idle)
    {
		...
		case eSaveTransfer_CreateDummyFile:
			{
				...
				CStorageManager::ESaveGameState saveState = StorageManager.SaveSaveData( &UIScene_LoadOrJoinMenu::CreateDummySaveDataCallback, lpParameter );
				if(saveState == CStorageManager::ESaveGame_Save)
				{
		...
		case eSaveTransfer_GetSavesInfo:
			{
				...
				CStorageManager::ESaveGameState eSGIStatus= StorageManager.GetSavesInfo(pClass->m_iPad,&UIScene_LoadOrJoinMenu::CrossSaveGetSavesInfoCallback,pClass,"save");
		...
		case eSaveTransfer_LoadSaveFromDisc:
		{
			...
			{
				CStorageManager::ESaveGameState eLoadStatus=StorageManager.LoadSaveData(&pSaveDetails->SaveInfoA[saveInfoIndex],&LoadCrossSaveDataCallback,pClass);
				if(eLoadStatus == CStorageManager::ESaveGame_Load)
				{
        ...
        case eSaveTransfer_Saving:
			{
				...
				while(StorageManager.GetSaveState() != CStorageManager::ESaveGame_Idle )
				{
					Sleep(10);
					StorageManager.Tick();
				}
        ...
        case eSaveTransfer_Error:
			{
				if(bSaveFileCreated)
				{
					...
					app.getRemoteStorage()->waitForStorageManagerIdle();
						CStorageManager::ESaveGameState eDeleteStatus = StorageManager.DeleteSaveData(&pSaveDetails->SaveInfoA[saveInfoIndex],UIScene_LoadOrJoinMenu::CrossSaveDeleteOnErrorReturned,pClass);
					if(eDeleteStatus == CStorageManager::ESaveGame_Delete)
					{
...
int UIScene_LoadOrJoinMenu::CrossSaveUploadFinishedCallback(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu *) pParam;
...
int UIScene_LoadOrJoinMenu::SaveTransferDialogReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)pParam;
	// results switched for this dialog
	if(result==CStorageManager::EMessage_ResultAccept) 
...
void UIScene_LoadOrJoinMenu::LaunchSaveTransfer()
{
	...
	stateContainer->m_eSaveTransferState = CStorageManager::eSaveTransfer_Idle;
...
int UIScene_LoadOrJoinMenu::DownloadXbox360SaveThreadProc( LPVOID lpParameter )
{
	...
	while(StorageManager.SaveTransferClearState()!=CStorageManager::eSaveTransfer_Idle)
	{
		Sleep(5);
	}
    ...
    while((pStateContainer->m_eSaveTransferState!=CStorageManager::eSaveTransfer_Idle) && pStateContainer->m_bSaveTransferInProgress && !pStateContainer->m_bSaveTransferCancelled)
    {
        switch(pStateContainer->m_eSaveTransferState)
        {
        case CStorageManager::eSaveTransfer_Idle:
            break;
        case CStorageManager::eSaveTransfer_FileSizeRetrieved:
            ...
            case eSaveTransferFile_Marker:
				if(UIScene_LoadOrJoinMenu::s_ulFileSize == 0)
				{
					...
					pStateContainer->m_eSaveTransferState=CStorageManager::eSaveTransfer_Idle;
				}
				else
				{
            ...
            break;
        case CStorageManager::eSaveTransfer_GettingFileData:

            break;
        case CStorageManager::eSaveTransfer_FileDataRetrieved:
            switch(UIScene_LoadOrJoinMenu::s_eSaveTransferFile)
            {
            case eSaveTransferFile_Marker:
...
#endif
                    pStateContainer->m_eSaveTransferState=CStorageManager::eSaveTransfer_Saving;
            ...
            break;
        case CStorageManager::eSaveTransfer_Converting:
#if 0
...
#endif
            break;
        case CStorageManager::eSaveTransfer_Saving:
            // On Durango/Orbis, we need to wait for all the asynchronous saving processes to complete before destroying the levels, as that will ultimately delete
            // the directory level storage & therefore the ConsoleSaveSplit instance, which needs to be around until all the sub files have completed saving.
			pMinecraft->progressRenderer->progressStage(IDS_PROGRESS_SAVING_TO_DISC);            
            ...
            //	pClass->m_eSaveTransferState=CStorageManager::eSaveTransfer_Idle;
            //	pMinecraft->progressRenderer->progressStage( IDS_SAVE_TRANSFER_DOWNLOAD_AND_CONVERT_COMPLETE );
...
{	
    ...
    {
        ZeroMemory(&m_debugTransferDetails, sizeof(CStorageManager::SAVETRANSFER_FILE_DETAILS) );

        File targetFile( wstring(L"FakeTMSPP\\").append(filename) );
        if(targetFile.exists()) m_debugTransferDetails.ulFileLen = (unsigned long)targetFile.length();
...
#endif
    {
        do
		{
			...
			pMinecraft->progressRenderer->progressStage( IDS_SAVETRANSFER_STAGE_GET_DETAILS );
			Sleep(1);
			pClass->m_eSaveTransferState=StorageManager.SaveTransferGetDetails(pClass->m_iPad,CStorageManager::eGlobalStorage_TitleUser,filename,&UIScene_LoadOrJoinMenu::SaveTransferReturned,pClass);
		}
		while(pClass->m_eSaveTransferState == CStorageManager::eSaveTransfer_Busy && !pClass->m_bSaveTransferCancelled );
    }
}
...
void UIScene_LoadOrJoinMenu::RequestFileData( SaveTransferStateContainer *pClass, wchar_t *filename )
{
    ...
    if(app.GetLoadSavesFromFolderEnabled())
    {
        ...
        {
            HANDLE hSaveFile = CreateFileW( targetFile.getPath().c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_RANDOM_ACCESS, NULL);
...
#endif
    {
        do
		{
			...
			Sleep(1);
			pClass->m_eSaveTransferState=StorageManager.SaveTransferGetData(pClass->m_iPad,CStorageManager::eGlobalStorage_TitleUser,filename,&UIScene_LoadOrJoinMenu::SaveTransferReturned,&UIScene_LoadOrJoinMenu::SaveTransferUpdateProgress,pClass,pClass);
		}
		while(pClass->m_eSaveTransferState == CStorageManager::eSaveTransfer_Busy && !pClass->m_bSaveTransferCancelled );
    }
}

int UIScene_LoadOrJoinMenu::SaveTransferReturned(LPVOID lpParam,CStorageManager::SAVETRANSFER_FILE_DETAILS *pSaveTransferDetails)
{
    ...
    {
        pClass->m_eSaveTransferState=CStorageManager::eSaveTransfer_FileSizeRetrieved;
    ...
    else
    {
        ...
        pClass->m_eSaveTransferState=CStorageManager::eSaveTransfer_FileDataRetrieved;
    }
...
int UIScene_LoadOrJoinMenu::NeedSyncMessageReturned(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
    UIScene_LoadOrJoinMenu *pClass = (UIScene_LoadOrJoinMenu *)pParam;
...
int UIScene_LoadOrJoinMenu::CopySaveDataReturned( LPVOID lpParameter, bool success, CStorageManager::ESaveGameState state )
{
	ui.EnterCallbackIdCriticalSection();
...
int UIScene_LoadOrJoinMenu::CopySaveErrorDialogFinishedCallback(void *pParam,int iPad,CStorageManager::EMessageResult result)
{
	ui.EnterCallbackIdCriticalSection();
