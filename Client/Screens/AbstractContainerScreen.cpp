...
void AbstractContainerScreen::mouseClicked(int x, int y, int buttonNum)
{
    ...
    if (buttonNum == 0 || buttonNum == 1)
	{
		...
		{
#ifdef _WINDOWS64
            // Use KBM shift detection for quick-move (Shift+Click like Bedrock)
            bool quickKey = slotId != AbstractContainerMenu::CLICKED_OUTSIDE &&
                (g_KBMInput.IsKeyDown(VK_LSHIFT) || g_KBMInput.IsKeyDown(VK_RSHIFT));
#else
...
{
    if (eventKey == Keyboard::KEY_ESCAPE || eventKey == Keyboard::KEY_E || eventKey == minecraft->options->keyBuild->key)
	{
